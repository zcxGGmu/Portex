...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. `main` currently includes `M8.5.13`; the immediate next step is post-`M8.5.13` search-experience refinement (for example optional result-ranking adjustments or time-range filters) while preserving existing terminal compatibility/RBAC boundaries.
...
- If the user explicitly chooses to continue post-`M8.5.13` terminal work, keep `latest.json`/`/sessions/current/history` compatibility and branch cleanly into search-experience refinements without changing current RBAC boundaries.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the `M8.5.13` terminal history search/filter-alignment focused suite on the latest `main` baseline.
