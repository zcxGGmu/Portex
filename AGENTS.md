...
- `main` currently contains the `M8.3` design/implementation-plan docs only (`c98e2fb`, `f63bb9d`), not the runtime/frontend implementation.
- Full `M8.3` implementation is complete on the local worktree branch `m8-3-terminal-operator-ux` via commits `819159d`, `cabc9ec`, `553eaf3`, `34c1792`, and `24485a9`, covering the terminal overview API, `/terminals` page, and minimal `/chat?workspace=...` deep link.
- Latest `M8.3` branch verification passed: `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_api_routes.py -q`, `cd web && npm run lint`, `cd web && npm run build`, `.venv/bin/pytest -o addopts='' -q` (`564 passed`), `.venv/bin/ruff check .`, and `git diff --check`.
...
- Latest completed web/terminal commits on `main` are `0fc394a` (`feat(web): complete M7.5.7 mobile pwa shell`), `dd16ee9` (`docs(parity): complete M7.6.1 qq scope decision`), `f52910e` (`docs(parity): complete M7.6.3 slash command decision`), `860d860` (`docs(parity): complete M7.6.4 richer im artifacts decision`), `e34e043` (`docs(parity): complete M7.6.5 unmatched surfaces decision`), `dc5858b` (`docs(terminal): plan M8.1 terminal session backend`), `aaeeb6d` (`feat(terminal): add M8.1 terminal session backend`), `51a0172` (`test(terminal): stabilize websocket detach assertion`), `bb00905` (`docs(web): plan M8.2 terminal ui`), and `a488e13` (`feat(web): complete M8.2 terminal ui`).
- Latest completed `M8.3` branch commits are `819159d` (`feat(terminal): add session listing helper`), `cabc9ec` (`feat(terminal): add terminal overview api`), `553eaf3` (`test(terminal): cover overview schema in openapi`), `34c1792` (`feat(web): complete M8.3 terminal overview`), and `24485a9` (`fix(web): polish M8.3 terminal overview states`).
...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. On `main`, the immediate next integration step is to merge or cherry-pick the local `m8-3-terminal-operator-ux` branch; after that, the natural continuation point becomes post-`M8.3` terminal control/fidelity work rather than earlier parity milestones.
...
- If resuming from the local branch `m8-3-terminal-operator-ux` after `M8.3`, also skim `docs/plans/2026-03-15-m8-3-terminal-operator-ux-design.md`, `docs/plans/2026-03-15-m8-3-terminal-operator-ux.md`, `services/terminal_sessions.py`, `app/routes/terminals.py`, `tests/app/routes/test_terminal_monitor_routes.py`, `web/src/pages/Terminals.tsx`, `web/src/components/chat/ChatPanel.tsx`, `web/src/api/client.ts`, and `web/src/hooks/useApi.ts` because the branch adds the read-only terminal overview surface and the minimal chat workspace deep-link behavior.
- If the user explicitly chooses to continue post-`M8.3` terminal work, first decide whether to integrate `m8-3-terminal-operator-ux` into `main` or keep working on that branch, then branch the next milestone cleanly into either terminal light-control actions or terminal fidelity/session-management work.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_api_routes.py -q`: run the `M8.3` terminal overview focused suite on the `m8-3-terminal-operator-ux` branch (or after merging it).
