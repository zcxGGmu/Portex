...
- `M8.5.18` is complete on `feat/m8-5-18-terminal-word-boundary-ranking` (`244da16`, `feat(terminal): add M8.5.18 word-boundary relevance`; `8a122d2`, `fix(terminal): stabilize M8.5.18 no-whole-word fallback ordering`); it remains a backend-only relevance refinement and is not yet merged back to `main`.
...
- Latest active planning commits on `main` are `e3d2d2f` (`docs(plans): add M8.5.17 terminal relevance ranking design`) and `372bf85` (`docs(plans): add M8.5.17 terminal relevance ranking implementation plan`).
- Latest restart-oriented handoff sync commit on this branch is `3c47abc` (`docs(handoff): sync M8.5.18 word-boundary relevance context`). If this line drifts from `git log`, treat `docs/progress.md` plus the latest functional milestone commits as the source of truth.
...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. `main` currently includes the merged `M8.5.17` baseline, while `feat/m8-5-18-terminal-word-boundary-ranking` includes the next backend-only `M8.5.18` word-boundary refinement; the immediate next step after this branch is a small post-`M8.5.18` search-quality refinement that preserves existing terminal compatibility/RBAC boundaries.
...
- If resuming after `M8.5.18`, also skim `docs/plans/2026-03-18-m8-5-17-terminal-relevance-ranking-design.md`, `docs/plans/2026-03-18-m8-5-17-terminal-relevance-ranking.md`, `docs/plans/2026-03-18-m8-5-18-terminal-word-boundary-relevance-design.md`, `docs/plans/2026-03-18-m8-5-18-terminal-word-boundary-relevance.md`, `services/terminal_sessions.py`, `app/routes/terminals.py`, `web/src/api/client.ts`, `web/src/hooks/useApi.ts`, `web/src/pages/Terminals.tsx`, `tests/services/test_terminal_sessions.py`, `tests/app/routes/test_terminal_routes.py`, and `tests/app/routes/test_api_routes.py` because `M8.5.18` keeps route/UI unchanged while refining backend `relevance` ordering with whole-word signals and stable no-whole-word fallback.
- If the user explicitly chooses to continue post-`M8.5.18` terminal work, keep `latest.json`/`/sessions/current/history` compatibility and branch cleanly into the next backend-only relevance-quality refinement without changing current RBAC boundaries.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the completed `M8.5.18` terminal focused suite (whole-word relevance + route/OpenAPI regression baseline).
