...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. The current worktree now includes the `M8.5.19` backend-only line-boundary relevance baseline; the immediate next step is another small post-`M8.5.19` backend-only search-quality refinement, likely line-start density or log-marker quality weighting, while preserving existing terminal compatibility/RBAC boundaries.
...
- If resuming after `M8.5.19`, also skim `docs/plans/2026-03-18-m8-5-17-terminal-relevance-ranking-design.md`, `docs/plans/2026-03-18-m8-5-17-terminal-relevance-ranking.md`, `docs/plans/2026-03-18-m8-5-18-terminal-word-boundary-relevance-design.md`, `docs/plans/2026-03-18-m8-5-18-terminal-word-boundary-relevance.md`, `docs/plans/2026-03-19-m8-5-19-terminal-line-boundary-relevance-design.md`, `docs/plans/2026-03-19-m8-5-19-terminal-line-boundary-relevance.md`, `services/terminal_sessions.py`, `app/routes/terminals.py`, `web/src/api/client.ts`, `web/src/hooks/useApi.ts`, `web/src/pages/Terminals.tsx`, `tests/services/test_terminal_sessions.py`, `tests/app/routes/test_terminal_routes.py`, and `tests/app/routes/test_api_routes.py` because `M8.5.19` keeps route/UI unchanged while refining backend `relevance` ordering with line-start whole-word signals and stable fallback to the existing `M8.5.18` signals.
- If the user explicitly chooses to continue post-`M8.5.19` terminal work, keep `latest.json`/`/sessions/current/history` compatibility and branch cleanly into the next backend-only relevance-quality refinement without changing current RBAC boundaries.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the completed `M8.5.19` terminal focused suite (line-boundary relevance + route/OpenAPI regression baseline).
