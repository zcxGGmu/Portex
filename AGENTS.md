...
## Current Baseline Snapshot (2026-03-19)
...
- Latest completed web/terminal commit in the current worktree is `cd2e80e` (`feat(terminal): add M8.5.23 exact-tag line-start relevance`).
- Latest active planning commit on the current branch is `940eac2` (`docs(plans): add M8.5.23 exact-tag line-start relevance plan`); that commit includes both the `M8.5.23` design and implementation-plan docs.
- Treat `docs/progress.md` plus the latest functional milestone commits as source of truth for the current `M8.5.23` baseline and exact commit order; use current-branch `git log` to locate the latest handoff-only sync commit when needed.
...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. The current worktree now includes the `M8.5.23` backend-only exact-tag line-start relevance baseline; the immediate next step is another small post-`M8.5.23` backend-only search-quality refinement, likely a raw line-start marker delimiter-boundary tie-break, while preserving existing terminal compatibility/RBAC boundaries. `M8.5.23` itself only adds narrow delimiter-aware wrapper exact-tag preference such as `[query] text`, `[query]: text`, and `[query] - text` on top of `M8.5.22`, and intentionally does not generalize to raw marker delimiter heuristics or arbitrary tokenization.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the completed `M8.5.23` terminal focused suite (exact-tag line-start relevance + route/OpenAPI regression baseline).
