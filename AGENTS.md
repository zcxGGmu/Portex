...
- `M7.2` is in progress in the parity backlog; the coordinator/policy core exists, but backend adapters and entry-point rewiring are still pending.
...
- The formal `M6` roadmap is exhausted. If the user continues parity work, the next real entrypoint is `M7.2.2`, not `M7.1`.
...
- If resuming during `M7.2`, also skim `services/execution_coordinator.py`, `services/execution_policy.py`, `services/group_queue.py`, `tests/services/test_execution_coordinator.py`, `tests/services/test_execution_policy.py`, `docs/plans/2026-03-12-m7-2-execution-plane-parity-design.md`, and `docs/plans/2026-03-12-m7-2-execution-plane-parity.md` because the coordinator/policy core is already present and the next real step is `M7.2.2` backend adapters plus entry-point rewiring.
- If the user explicitly chooses to continue parity work past `M7.1`, resume from `M7.2.2` after skimming `services/execution_coordinator.py`, `services/execution_policy.py`, `services/group_queue.py`, `tests/services/test_execution_coordinator.py`, `tests/services/test_execution_policy.py`, `docs/plans/2026-03-12-m7-2-execution-plane-parity-design.md`, and `docs/plans/2026-03-12-m7-2-execution-plane-parity.md`.
...
- `.venv/bin/pytest -o addopts='' tests/services/test_execution_coordinator.py tests/services/test_execution_policy.py -q`: run the current `M7.2` coordinator/policy core suite.
