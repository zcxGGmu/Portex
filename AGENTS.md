...
- `M8.5.17` is complete on `feat/m8-5-17-terminal-relevance-ranking` (`a40fdda`, `feat(terminal): refine M8.5.17 relevance ranking`); it remains a backend-only relevance-ranking refinement and is not yet merged back to `main`.
...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. `main` currently includes the `M8.5.16` functional baseline plus `M8.5.17` planning docs, while `feat/m8-5-17-terminal-relevance-ranking` already includes the completed backend-only `M8.5.17` relevance-ranking refinement; the immediate next step after this branch is a small post-`M8.5.17` search-quality refinement that preserves existing terminal compatibility/RBAC boundaries.
...
- If resuming after `M8.5.17`, also skim `docs/plans/2026-03-18-m8-5-16-terminal-search-sort-controls-design.md`, `docs/plans/2026-03-18-m8-5-16-terminal-search-sort-controls.md`, `docs/plans/2026-03-18-m8-5-17-terminal-relevance-ranking-design.md`, `docs/plans/2026-03-18-m8-5-17-terminal-relevance-ranking.md`, `services/terminal_sessions.py`, `app/routes/terminals.py`, `web/src/api/client.ts`, `web/src/hooks/useApi.ts`, `web/src/pages/Terminals.tsx`, `tests/services/test_terminal_sessions.py`, `tests/app/routes/test_terminal_routes.py`, and `tests/app/routes/test_api_routes.py` because `M8.5.17` keeps the route/UI surface unchanged while refining backend `relevance` ordering through cluster span, first-match position, density, and weak recency tie-breaks.
- If the user explicitly chooses to continue post-`M8.5.17` terminal work, keep `latest.json`/`/sessions/current/history` compatibility and branch cleanly into the next backend-only relevance-quality refinement without changing current RBAC boundaries.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the completed `M8.5.17` terminal focused suite (service heuristics + route/OpenAPI regression baseline).
