...
- The formal `M6` roadmap is exhausted. If the user continues parity work, the next real entrypoint is `M7.2`, not `M7.1`.
...
- If the user explicitly chooses to continue parity work past `M7.1`, start by planning `M7.2`; do not reopen `M7.1` scope unless a concrete regression or follow-up bug is identified.
