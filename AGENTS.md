...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete, and the current natural continuation point is post-`M8.2` terminal fidelity / operator UX work rather than earlier parity milestones.
...
- If the user explicitly chooses to continue post-`M8.2` terminal work, resume from a new terminal fidelity / operator UX milestone after skimming the `M8.1` and `M8.2` design docs plus the backend/frontend terminal files listed above.
