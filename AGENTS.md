...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. On `main`, the immediate continuation point is post-`M8.5.7` timeline filtering/search/detail work rather than earlier parity milestones.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the `M8.5.7` terminal inventory/timeline focused suite on `main`.
