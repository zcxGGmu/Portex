...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. `main` currently includes `M8.5.14`; the immediate next step is post-`M8.5.14` search-experience refinement (for example optional result-ranking adjustments or preset time-range filters) while preserving existing terminal compatibility/RBAC boundaries.
...
- If resuming after `M8.5.14`, also skim `docs/plans/2026-03-17-m8-5-14-terminal-time-range-filters-design.md`, `docs/plans/2026-03-17-m8-5-14-terminal-time-range-filters.md`, `services/terminal_sessions.py`, `app/routes/terminals.py`, `web/src/api/client.ts`, `web/src/hooks/useApi.ts`, `web/src/pages/Terminals.tsx`, `tests/services/test_terminal_sessions.py`, `tests/app/routes/test_terminal_routes.py`, and `tests/app/routes/test_api_routes.py` because `M8.5.14` adds inclusive time-range filtering on `snapshot_at` plus shared timeline/search time controls.
- If the user explicitly chooses to continue post-`M8.5.14` terminal work, keep `latest.json`/`/sessions/current/history` compatibility and branch cleanly into search-experience refinements without changing current RBAC boundaries.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the `M8.5.14` terminal history search/time-range focused suite on the latest `main` baseline.
