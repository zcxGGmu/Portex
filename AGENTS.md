...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. `main` currently includes the merged `M8.5.18` baseline; the immediate next step is a small post-`M8.5.18` backend-only search-quality refinement, likely line-boundary weighting, while preserving existing terminal compatibility/RBAC boundaries.
