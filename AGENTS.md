...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. The current worktree now includes the `M8.5.20` backend-only line-start quality relevance baseline; the immediate next step is another small post-`M8.5.20` backend-only search-quality refinement, likely log-marker quality or punctuation-aware line-start weighting, while preserving existing terminal compatibility/RBAC boundaries.
...
- If resuming after `M8.5.20`, also skim `docs/plans/2026-03-18-m8-5-17-terminal-relevance-ranking-design.md`, `docs/plans/2026-03-18-m8-5-17-terminal-relevance-ranking.md`, `docs/plans/2026-03-18-m8-5-18-terminal-word-boundary-relevance-design.md`, `docs/plans/2026-03-18-m8-5-18-terminal-word-boundary-relevance.md`, `docs/plans/2026-03-19-m8-5-19-terminal-line-boundary-relevance-design.md`, `docs/plans/2026-03-19-m8-5-19-terminal-line-boundary-relevance.md`, `docs/plans/2026-03-19-m8-5-20-terminal-line-start-quality-relevance-design.md`, `docs/plans/2026-03-19-m8-5-20-terminal-line-start-quality-relevance.md`, `services/terminal_sessions.py`, `app/routes/terminals.py`, `web/src/api/client.ts`, `web/src/hooks/useApi.ts`, `web/src/pages/Terminals.tsx`, `tests/services/test_terminal_sessions.py`, `tests/app/routes/test_terminal_routes.py`, and `tests/app/routes/test_api_routes.py` because `M8.5.20` keeps route/UI unchanged while refining backend `relevance` ordering with conditional non-line-start whole-word noise weighting on top of the existing `M8.5.19` line-start signals.
- If the user explicitly chooses to continue post-`M8.5.20` terminal work, keep `latest.json`/`/sessions/current/history` compatibility and branch cleanly into the next backend-only relevance-quality refinement without changing current RBAC boundaries.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the completed `M8.5.20` terminal focused suite (line-start quality relevance + route/OpenAPI regression baseline).
