...
- Current starting point is `M6.5.2` (Release tag creation).
- Planned first release tag is `v1.0.0`; package/runtime version intentionally remains `0.1.0` until `M6.5.2+`.
...
- If resuming after `M6.5.1`, also skim `docs/plans/2026-03-11-m6-5-1-version-planning-design.md`, `docs/plans/2026-03-11-m6-5-1-version-planning.md`, `pyproject.toml`, `README.md`, and `docs/progress.md` because the current release plan targets tag `v1.0.0` while intentionally leaving package/runtime version strings at `0.1.0` until `M6.5.2+`.
