...
- Latest active planning commit on the current branch is `a4154a7` (`docs(plans): add M8.5.21 log-marker relevance plan`); that commit includes both the `M8.5.21` design and implementation-plan docs.
- Treat `docs/progress.md` plus the latest functional milestone commits as source of truth for the current `M8.5.21` baseline and exact commit order; use current-branch `git log` to locate the latest handoff-only sync commit when needed.
...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. The current worktree now includes the `M8.5.21` backend-only log-marker relevance baseline; the immediate next step is another small post-`M8.5.21` backend-only search-quality refinement, likely punctuation-aware line-start weighting, while preserving existing terminal compatibility/RBAC boundaries. `M8.5.21` itself only recognizes narrow line-start `query:` and strict `query -` markers, and intentionally does not generalize to `[query]` or broader punctuation heuristics.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the completed `M8.5.21` terminal focused suite (log-marker relevance + route/OpenAPI regression baseline).
