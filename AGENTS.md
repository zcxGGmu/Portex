...
- Latest completed web/terminal commits on `main` include `af095cc` (`feat(terminal): complete M8.3 operator overview on main`), `c9c47ec` (`feat(terminal): complete M8.4 light controls`), `901393d` (`feat(terminal): complete M8.5.1 output replay`), `87322e1` (`feat(terminal): complete M8.5.2 resize fidelity`), `010cc40` (`feat(terminal): complete M8.5.3 history read surface`), `0563192` (`feat(terminal): complete M8.5.4 history persistence fallback`), `a799238` (`feat(terminal): complete M8.5.5 active session recovery`), and `ffcf571` (`feat(terminal): complete M8.5.6 history inventory overview`).
...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. On `main`, the immediate continuation point is post-`M8.5.6` terminal history timeline/pagination work (multi-snapshot inventory) rather than earlier parity milestones.
...
- If resuming after `M8.5.6`, also skim `docs/plans/2026-03-16-m8-5-6-terminal-history-inventory-design.md`, `docs/plans/2026-03-16-m8-5-6-terminal-history-inventory.md`, `services/terminal_sessions.py`, `app/routes/terminals.py`, `domain/schemas.py`, `web/src/pages/Terminals.tsx`, `tests/services/test_terminal_sessions.py`, `tests/app/routes/test_terminal_monitor_routes.py`, and `tests/app/routes/test_api_routes.py` because `M8.5.6` completes persistence-aware history inventory and defines the current post-`M8.5.6` starting boundary.
- If the user explicitly chooses to continue post-`M8.5.6` terminal work, branch the next milestone cleanly into multi-snapshot history timeline/pagination without regressing current `latest.json` persistence compatibility.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the `M8.5.6` terminal inventory/history focused suite on `main`.
