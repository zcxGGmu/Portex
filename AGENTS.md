...
- Latest completed web/terminal commit in the current worktree is `f543d77` (`feat(terminal): add M8.5.22 punctuation-aware line-start relevance`).
- Latest active planning commit on the current branch is `ab35391` (`docs(plans): add M8.5.22 punctuation-aware line-start relevance plan`); that commit includes both the `M8.5.22` design and implementation-plan docs.
- Treat `docs/progress.md` plus the latest functional milestone commits as source of truth for the current `M8.5.22` baseline and exact commit order; use current-branch `git log` to locate the latest handoff-only sync commit when needed.
...
- The formal `M6` roadmap is exhausted. The parity backlog through `M7.6.5` is complete. The current worktree now includes the `M8.5.22` backend-only punctuation-aware line-start relevance baseline; the immediate next step is another small post-`M8.5.22` backend-only search-quality refinement, likely a delimiter-aware / exact-tag line-start tie-break, while preserving existing terminal compatibility/RBAC boundaries. `M8.5.22` itself only adds narrow single-character line-start wrappers `[query]`, `(query)`, `{query}`, and `<query>` on top of `M8.5.21`, and intentionally does not generalize to arbitrary punctuation heuristics.
...
- `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`: run the completed `M8.5.22` terminal focused suite (punctuation-aware line-start relevance + route/OpenAPI regression baseline).
