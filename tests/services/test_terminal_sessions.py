...
@pytest.mark.asyncio
async def test_terminal_session_service_lists_latest_history_snapshots_with_persisted_fallback(
    tmp_path: Path,
) -> None:
    from services.terminal_sessions import TerminalSessionService
    ...
    items = second_service.list_latest_history_snapshots()

    assert [item.record.group_folder for item in items] == ["project-alpha", "project-beta"]
    assert items[0].record.status == "closed"
    assert items[0].output == "alpha-output\n"
    assert items[1].record.status == "attached"
    assert items[1].output == "beta-output\n"


@pytest.mark.asyncio
async def test_terminal_session_service_lists_history_snapshot_archives_by_groups(
    tmp_path: Path,
) -> None:
    from services.terminal_sessions import TerminalSessionService
    ...
    archives = await second_service.list_history_snapshot_archives_by_groups(
...
@pytest.mark.asyncio
async def test_terminal_session_service_lists_history_snapshot_page_with_pagination(
    tmp_path: Path,
) -> None:
    from services.terminal_sessions import TerminalSessionService
    ...
    first_page = await service.list_history_snapshot_page_by_group("project-alpha", limit=1, offset=0)
    second_page = await service.list_history_snapshot_page_by_group("project-alpha", limit=1, offset=1)
    ...
    assert len(first_page.items) == 1
    assert first_page.items[0].record.session_id == second.session_id
    assert first_page.items[0].output == "second\n"
    ...
    assert len(second_page.items) == 1
    assert second_page.items[0].record.session_id == first.session_id
    assert second_page.items[0].output == "first\n"
...
@pytest.mark.asyncio
async def test_terminal_session_service_snapshot_page_raises_for_missing_workspace() -> None:
    from services.terminal_sessions import TerminalSessionNotFoundError, TerminalSessionService

    service = TerminalSessionService(bridge_factory=lambda **_: FakeBridge())

    with pytest.raises(TerminalSessionNotFoundError, match="terminal session not found"):
        await service.list_history_snapshot_page_by_group("missing-workspace", limit=20, offset=0)
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_keeps_log_markers_above_punctuation_wrappers_and_wrappers_above_plain_line_start_hits(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    bracketed_line_start = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _bracketed_record, bracketed_queue = await service.attach_session(
        bracketed_line_start.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error] aa\nterror zz\n")
    await asyncio.wait_for(bracketed_queue.get(), timeout=0.1)
    await service.close_session(bracketed_line_start.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_line_start_punctuation_wrap_offset_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_wrapper = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_wrapper.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error] aa\nmid error here\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_wrapper.session_id, owner_user_id="owner-1")

    later_wrapper = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_wrapper.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("mid error here\n[error] aa\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_wrapper.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_21_signals_when_no_punctuation_wrappers_exist(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_punctuation_wrap_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_delimited_exact_tag_wrappers_over_tight_wrappers(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_line_start_exact_tag_offset_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_exact_tag = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_exact_tag.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error] aa\nmid error here\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_exact_tag.session_id, owner_user_id="owner-1")

    later_exact_tag = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_exact_tag.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("mid error here\n[error] aa\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_exact_tag.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_22_signals_when_no_exact_tag_wrapper_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_tight_wrapper = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_tight_wrapper.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]aa\nmid error here\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_tight_wrapper.session_id, owner_user_id="owner-1")

    later_tight_wrapper = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_tight_wrapper.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("mid error here\n[error]aa\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_tight_wrapper.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_exact_tag_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_exact_tag_markers_over_plain_exact_tags(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 20, 16, 0, tzinfo=timezone.utc)
    ...
    exact_tag_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    await created_bridges[0].emit_output("[error]: aa\n")
    await asyncio.wait_for(marker_queue.get(), timeout=0.1)
    await service.close_session(exact_tag_marker.session_id, owner_user_id="owner-1")

    plain_exact_tag = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    await created_bridges[1].emit_output("[error] aa\n")
    await asyncio.wait_for(plain_queue.get(), timeout=0.1)
    await service.close_session(plain_exact_tag.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_line_start_exact_tag_marker_offset_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_24_signals_when_no_exact_tag_markers_exist(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_exact_tag_marker_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_square_bracket_exact_tags_over_other_wrapper_exact_tags(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 20, 18, 0, tzinfo=timezone.utc)
    ...
    square_bracket_exact_tag = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _square_record, square_queue = await service.attach_session(
        square_bracket_exact_tag.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error] aa\n")
    await asyncio.wait_for(square_queue.get(), timeout=0.1)
    await service.close_session(square_bracket_exact_tag.session_id, owner_user_id="owner-1")

    paren_exact_tag = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _paren_record, paren_queue = await service.attach_session(
        paren_exact_tag.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("(error) aa\n")
    await asyncio.wait_for(paren_queue.get(), timeout=0.1)
    await service.close_session(paren_exact_tag.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_square_bracket_exact_tag_offset_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_square = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_square.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error] aa\nmid error here\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_square.session_id, owner_user_id="owner-1")

    later_square = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_square.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("mid error here\n[error] aa\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_square.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_25_signals_when_no_square_bracket_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_square_bracket_exact_tag_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_brace_plain_exact_tags_over_angle_plain_exact_tags(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 21, 2, 0, tzinfo=timezone.utc)
    ...
    brace_exact_tag = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _brace_record, brace_queue = await service.attach_session(
        brace_exact_tag.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("{error} aa\n")
    await asyncio.wait_for(brace_queue.get(), timeout=0.1)
    await service.close_session(brace_exact_tag.session_id, owner_user_id="owner-1")

    angle_exact_tag = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _angle_record, angle_queue = await service.attach_session(
        angle_exact_tag.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("<error> aa\n")
    await asyncio.wait_for(angle_queue.get(), timeout=0.1)
    await service.close_session(angle_exact_tag.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_paren_plain_exact_tags_over_angle_plain_exact_tags(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_paren_plain_exact_tags_over_brace_plain_exact_tags(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 21, 3, 0, tzinfo=timezone.utc)
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_paren_plain_exact_tag_offset_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_paren = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_paren.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("{error} aa\n(error) bb\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_paren.session_id, owner_user_id="owner-1")

    later_paren = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_paren.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("{error} aa\npadding\n(error) bb\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_paren.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_brace_plain_exact_tag_offset_when_stronger_signals_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 21, 5, 0, tzinfo=timezone.utc)
    ...
    earlier_brace = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_brace.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n{error} bb\n<error> cc\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_brace.session_id, owner_user_id="owner-1")

    later_brace = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_brace.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n<error> bb\n{error} cc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_brace.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_35_signals_when_no_brace_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_no_brace_plain = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_no_brace_plain.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n<error> bb\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_no_brace_plain.session_id, owner_user_id="owner-1")

    later_no_brace_plain = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_no_brace_plain.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("padding\n[error]: aa\n<error> bb\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_no_brace_plain.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_brace_plain_exact_tag_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    await created_bridges[2].emit_output("<error>cc\nmid error here\n")
    await asyncio.wait_for(tight_queue.get(), timeout=0.1)
    await service.close_session(tight_wrapper.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_angle_plain_exact_tag_offset_when_stronger_signals_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 21, 6, 0, tzinfo=timezone.utc)
    ...
    earlier_angle = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_angle.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n<error> bb\npadding\n<error> cc\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_angle.session_id, owner_user_id="owner-1")

    later_angle = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_angle.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\npadding\n<error> bb\n<error> cc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_angle.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_36_signals_when_no_angle_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_no_angle_plain = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_no_angle_plain.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n{error} bb\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_no_angle_plain.session_id, owner_user_id="owner-1")

    later_no_angle_plain = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_no_angle_plain.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\npadding\n{error} bb\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_no_angle_plain.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_square_bracket_plain_exact_tag_offset_when_stronger_signals_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 21, 7, 0, tzinfo=timezone.utc)
    ...
    earlier_square_plain = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_square_plain.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] bb\npadding\n[error] cc\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_square_plain.session_id, owner_user_id="owner-1")

    later_square_plain = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_square_plain.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\npadding\n[error] bb\n[error] cc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_square_plain.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_37_signals_when_no_square_bracket_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_no_square_plain = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_no_square_plain.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n<error> bb\npadding\n<error> cc\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_no_square_plain.session_id, owner_user_id="owner-1")

    later_no_square_plain = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_no_square_plain.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\npadding\n<error> bb\n<error> cc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_no_square_plain.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_square_bracket_plain_exact_tag_offset_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_more_single_space_plain_exact_tag_matches_when_stronger_signals_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 21, 8, 0, tzinfo=timezone.utc)
    ...
    cleaner_separator = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _cleaner_record, cleaner_queue = await service.attach_session(
        cleaner_separator.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] bb\n[error] cc\n")
    await asyncio.wait_for(cleaner_queue.get(), timeout=0.1)
    await service.close_session(cleaner_separator.session_id, owner_user_id="owner-1")

    noisier_separator = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _noisier_record, noisier_queue = await service.attach_session(
        noisier_separator.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n[error]\tbb\n[error] cc\n")
    await asyncio.wait_for(noisier_queue.get(), timeout=0.1)
    await service.close_session(noisier_separator.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_38_signals_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_no_single_space = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_no_single_space.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error]\tbb\npadding\n[error]\tcc\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_no_single_space.session_id, owner_user_id="owner-1")

    later_no_single_space = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_no_single_space.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\npadding\n[error]\tbb\n[error]\tcc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_no_single_space.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_single_space_plain_exact_tag_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_later_non_single_space_plain_exact_tag_noise_when_stronger_signals_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 22, 1, 0, tzinfo=timezone.utc)
    ...
    later_noise = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_noise.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] ok\npadding\n[error]\tok\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_noise.session_id, owner_user_id="owner-1")

    earlier_noise = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_noise.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n[error] ok\n[error]\tok\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_noise.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_39_signals_when_no_single_space_separator_noise_tie_break_applies(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_separator_noise_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_later_payloadless_plain_exact_tag_separator_offsets_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    later_payloadless = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_payloadless.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] ok\n[error]\to\n[error]\t \n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_payloadless.session_id, owner_user_id="owner-1")

    earlier_payloadless = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_payloadless.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n[error] ok\n[error]\t \n[error]\to\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_payloadless.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_payloadless_offset_tie_break_falls_back_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_payloadless_offset_tie_break_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_fewer_tab_prefixed_payload_plain_exact_tag_separators_when_stronger_signals_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 22, 3, 0, tzinfo=timezone.utc)
    ...
    without_tab_payload = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _without_tab_record, without_tab_queue = await service.attach_session(
        without_tab_payload.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] ok\n[error]  bb\n")
    await asyncio.wait_for(without_tab_queue.get(), timeout=0.1)
    await service.close_session(without_tab_payload.session_id, owner_user_id="owner-1")

    with_tab_payload = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _with_tab_record, with_tab_queue = await service.attach_session(
        with_tab_payload.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n[error] ok\n[error]\tbb\n")
    await asyncio.wait_for(with_tab_queue.get(), timeout=0.1)
    await service.close_session(with_tab_payload.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_tab_prefixed_payload_tie_break_falls_back_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_tab_prefixed_payload_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_later_tab_prefixed_payload_separator_offsets_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 22, 4, 0, tzinfo=timezone.utc)
    ...
    later_tab_payload = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_tab_payload.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] ok\n[error]  bb\npadding\n[error]\tcc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_tab_payload.session_id, owner_user_id="owner-1")

    earlier_tab_payload = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_tab_payload.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n[error] ok\n[error]  bb\n[error]\tcc\npadding\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_tab_payload.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_tab_prefixed_payload_offset_tie_break_falls_back_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_tab_prefixed_payload_offset_tie_break_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_later_multi_space_payload_separator_offsets_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 22, 6, 0, tzinfo=timezone.utc)
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_multi_space_payload_offset_tie_break_falls_back_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_multi_space_payload_offset_tie_break_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_fewer_space_prefixed_mixed_whitespace_payload_plain_exact_tag_separators_when_stronger_signals_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 22, 7, 0, tzinfo=timezone.utc)
    ...
    fewer_mixed = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _fewer_record, fewer_queue = await service.attach_session(
        fewer_mixed.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] ok\n[error] \tbb\n[error]\f cc\n")
    await asyncio.wait_for(fewer_queue.get(), timeout=0.1)
    await service.close_session(fewer_mixed.session_id, owner_user_id="owner-1")

    more_mixed = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _more_record, more_queue = await service.attach_session(
        more_mixed.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n[error] ok\n[error] \tbb\n[error] \fcc\n")
    await asyncio.wait_for(more_queue.get(), timeout=0.1)
    await service.close_session(more_mixed.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_space_prefixed_mixed_whitespace_payload_tie_break_falls_back_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_mixed = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_mixed.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] \tbb\npadding\n[error] \fcc\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_mixed.session_id, owner_user_id="owner-1")

    later_mixed = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_mixed.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\npadding\n[error] \tbb\n[error] \fcc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_mixed.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_space_prefixed_mixed_whitespace_payload_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_later_space_prefixed_mixed_whitespace_payload_separator_offsets_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 22, 8, 0, tzinfo=timezone.utc)
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_space_prefixed_mixed_whitespace_payload_offset_tie_break_falls_back_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_space_prefixed_mixed_whitespace_payload_offset_tie_break_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_later_other_leading_whitespace_payload_separator_offsets_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 22, 9, 0, tzinfo=timezone.utc)
    ...
    later_other = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_other.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] ok\n[error]\tbb\npadding\n[error]\vcc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_other.session_id, owner_user_id="owner-1")

    earlier_other = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_other.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n[error] ok\n[error]\tbb\n[error]\vcc\npadding\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_other.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_other_leading_whitespace_payload_offset_tie_break_falls_back_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_other_leading_whitespace_payload_offset_tie_break_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_fewer_other_leading_mixed_whitespace_payload_plain_exact_tag_separators_when_stronger_signals_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 22, 10, 0, tzinfo=timezone.utc)
    ...
    cleaner_snapshot = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _cleaner_record, cleaner_queue = await service.attach_session(
        cleaner_snapshot.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error] ok\n[error]\vab\n[error]\fcd\n")
    await asyncio.wait_for(cleaner_queue.get(), timeout=0.1)
    await service.close_session(cleaner_snapshot.session_id, owner_user_id="owner-1")

    noisier_snapshot = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _noisier_record, noisier_queue = await service.attach_session(
        noisier_snapshot.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n[error] ok\n[error]\v a\n[error]\f c\n")
    await asyncio.wait_for(noisier_queue.get(), timeout=0.1)
    await service.close_session(noisier_snapshot.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_other_leading_mixed_whitespace_payload_tie_break_falls_back_to_m8_5_49_signals_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_no_single_noisier = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_no_single_noisier.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n[error]\v\tbb\n[error]\f dd\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_no_single_noisier.session_id, owner_user_id="owner-1")

    later_no_single_cleaner = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_no_single_cleaner.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\npadding\n[error]\vbb\n[error]\fdd\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_no_single_cleaner.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_other_leading_mixed_whitespace_payload_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_later_other_leading_mixed_whitespace_payload_separator_offsets_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 22, 11, 0, tzinfo=timezone.utc)
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_other_leading_mixed_whitespace_payload_offset_tie_break_falls_back_when_no_single_space_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_other_leading_mixed_whitespace_payload_offset_tie_break_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_paren_plain_exact_tag_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_clean_exact_tags_over_exact_tags_with_tighter_wrapper_noise(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 21, 4, 0, tzinfo=timezone.utc)
    ...
    clean_exact_tag = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _clean_record, clean_queue = await service.attach_session(
        clean_exact_tag.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("{error} aa\nterror zz\n")
    await asyncio.wait_for(clean_queue.get(), timeout=0.1)
    await service.close_session(clean_exact_tag.session_id, owner_user_id="owner-1")

    noisy_exact_tag = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _noisy_record, noisy_queue = await service.attach_session(
        noisy_exact_tag.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("{error} aa\n{error}aa\n")
    await asyncio.wait_for(noisy_queue.get(), timeout=0.1)
    await service.close_session(noisy_exact_tag.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_fewer_exact_tag_punctuation_noises_when_stronger_signals_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    one_noise = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _one_record, one_queue = await service.attach_session(
        one_noise.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("{error} aa\n{error}aa\nterror zz\n")
    await asyncio.wait_for(one_queue.get(), timeout=0.1)
    await service.close_session(one_noise.session_id, owner_user_id="owner-1")

    two_noises = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _two_record, two_queue = await service.attach_session(
        two_noises.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("{error} aa\n{error}aa\n{error}bb\n")
    await asyncio.wait_for(two_queue.get(), timeout=0.1)
    await service.close_session(two_noises.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_34_signals_when_no_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_exact_tag_punctuation_noise_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_32_signals_when_no_angle_plain_exact_tag_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    brace_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _brace_record, brace_queue = await service.attach_session(
        brace_marker.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("{error}: aa\n")
    await asyncio.wait_for(brace_queue.get(), timeout=0.1)
    await service.close_session(brace_marker.session_id, owner_user_id="owner-1")

    angle_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _angle_record, angle_queue = await service.attach_session(
        angle_marker.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("<error>: aa\n")
    await asyncio.wait_for(angle_queue.get(), timeout=0.1)
    await service.close_session(angle_marker.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_exact_tag_colon_markers_over_exact_tag_dash_markers(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 20, 20, 0, tzinfo=timezone.utc)
    ...
    colon_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _colon_record, colon_queue = await service.attach_session(
        colon_marker.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa \n")
    await asyncio.wait_for(colon_queue.get(), timeout=0.1)
    await service.close_session(colon_marker.session_id, owner_user_id="owner-1")

    dash_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _dash_record, dash_queue = await service.attach_session(
        dash_marker.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error] - aa\n")
    await asyncio.wait_for(dash_queue.get(), timeout=0.1)
    await service.close_session(dash_marker.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_exact_tag_colon_marker_offset_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_colon = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_colon.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error] - aa\n[error]: bb \npadding\n[error] - cc\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_colon.session_id, owner_user_id="owner-1")

    later_colon = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_colon.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error] - aa\npadding\n[error]: bb \n[error] - cc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_colon.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_26_signals_when_no_exact_tag_colon_marker_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_exact_tag_colon_marker_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_square_bracket_exact_tag_dash_markers_over_generic_exact_tag_noise(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 20, 21, 0, tzinfo=timezone.utc)
    ...
    square_bracket_dash_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _square_record, square_queue = await service.attach_session(
        square_bracket_dash_marker.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("prefix error aa\n[error] - bb\n")
    await asyncio.wait_for(square_queue.get(), timeout=0.1)
    await service.close_session(square_bracket_dash_marker.session_id, owner_user_id="owner-1")

    generic_exact_tag_noise = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    )
    _generic_record, generic_queue = await service.attach_session(
        generic_exact_tag_noise.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("(error) - aa\n[error] bb\n")
    await asyncio.wait_for(generic_queue.get(), timeout=0.1)
    await service.close_session(generic_exact_tag_noise.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_square_bracket_exact_tag_dash_marker_offset_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    earlier_square_bracket_dash_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_square_bracket_dash_marker.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error] aa\n[error] - bb\n(error) - cc\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_square_bracket_dash_marker.session_id, owner_user_id="owner-1")

    later_square_bracket_dash_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_square_bracket_dash_marker.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("(error) - aa\n[error] bb\n[error] - cc\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_square_bracket_dash_marker.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_27_signals_when_no_square_bracket_exact_tag_dash_marker_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    paren_dash_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _paren_record, paren_queue = await service.attach_session(
        paren_dash_marker.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("(error) - aa\n")
    await asyncio.wait_for(paren_queue.get(), timeout=0.1)
    await service.close_session(paren_dash_marker.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_square_bracket_exact_tag_dash_marker_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_non_square_bracket_exact_tag_colon_markers_when_shared_square_bracket_colon_markers_tie_stronger_signals(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 20, 22, 0, tzinfo=timezone.utc)
    ...
    earlier_non_square_colon = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _earlier_record, earlier_queue = await service.attach_session(
        earlier_non_square_colon.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("[error]: aa\n(error): bbb\n(error) - bb\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_non_square_colon.session_id, owner_user_id="owner-1")

    later_non_square_colon = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _later_record, later_queue = await service.attach_session(
        later_non_square_colon.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[1].emit_output("[error]: aa\n(error) - bb\n(error): bbb\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_non_square_colon.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_non_square_bracket_exact_tag_colon_marker_offset_when_shared_square_bracket_colon_markers_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_28_signals_when_no_non_square_bracket_exact_tag_colon_marker_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_non_square_bracket_exact_tag_colon_marker_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_non_square_bracket_exact_tag_dash_marker_when_shared_square_bracket_dash_markers_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 21, 0, 0, tzinfo=timezone.utc)
    ...
    earlier_non_square_dash = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    await created_bridges[0].emit_output("[error] - aa\n(error) - bbb\n(error) bb\n")
    await asyncio.wait_for(earlier_queue.get(), timeout=0.1)
    await service.close_session(earlier_non_square_dash.session_id, owner_user_id="owner-1")

    later_non_square_dash = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    await created_bridges[1].emit_output("[error] - aa\n(error) bb\n(error) - bbb\n")
    await asyncio.wait_for(later_queue.get(), timeout=0.1)
    await service.close_session(later_non_square_dash.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_non_square_bracket_exact_tag_dash_marker_offset_when_shared_square_bracket_dash_markers_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_29_signals_when_no_non_square_bracket_exact_tag_dash_marker_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_non_square_bracket_exact_tag_dash_marker_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_paren_wrapper_markers_over_other_non_square_wrapper_markers(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
    ...
    current_time = datetime(2026, 3, 21, 1, 0, tzinfo=timezone.utc)
    ...
    paren_marker = await service.create_session(
        group_id="project-alpha",
        group_folder="project-alpha",
        owner_user_id="owner-1",
        requested_mode="container",
    ...
    _paren_record, paren_queue = await service.attach_session(
        paren_marker.session_id,
        owner_user_id="owner-1",
    )
    await created_bridges[0].emit_output("(error): aa\n")
    await asyncio.wait_for(paren_queue.get(), timeout=0.1)
    await service.close_session(paren_marker.session_id, owner_user_id="owner-1")
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_paren_wrapper_marker_offset_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_30_signals_when_no_paren_wrapper_marker_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_paren_wrapper_marker_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_brace_wrapper_markers_over_angle_wrapper_markers(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_prefers_earlier_brace_wrapper_marker_offset_when_counts_tie(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_falls_back_to_m8_5_31_signals_when_no_brace_wrapper_marker_exists(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
...
@pytest.mark.asyncio
async def test_terminal_session_service_relevance_brace_wrapper_marker_pagination_uses_global_ordering(
    tmp_path: Path,
) -> None:
    from datetime import datetime, timedelta, timezone
