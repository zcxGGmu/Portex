...
def _login_token(api_client: TestClient, *, username: str, role: str = "owner") -> tuple[str, str]:
    from services.auth import auth_service
...
def test_terminal_websocket_accepts_query_param_access_token_for_browser_clients(api_client: TestClient) -> None:
    from app.main import app
    from app.routes import terminals as terminal_routes
    from services.terminal_sessions import TerminalSessionRecord

    token, owner_id = _login_token(api_client, username="browser-owner", role="owner")

    class FakeTerminalService:
        async def attach_session(self, session_id: str, *, owner_user_id: str):
            from datetime import datetime, timezone

            assert session_id == "session-browser"
            assert owner_user_id == owner_id
            ...
            return (
                TerminalSessionRecord(
                    ...
                    owner_user_id=owner_id,
                    backend="docker_container",
                    container_name="portex-terminal-project-alpha-browser",
                    status="attached",
                    created_at=datetime(2026, 3, 15, 12, 0, tzinfo=timezone.utc),
                    last_attached_at=datetime(2026, 3, 15, 12, 1, tzinfo=timezone.utc),
                ),
    ...
    try:
        with api_client.websocket_connect(f"/ws/terminals/session-browser?access_token={token}") as websocket:
            ready_payload = json.loads(websocket.receive_text())
            websocket.close()
    finally:
        app.dependency_overrides.clear()
    ...
    assert ready_payload["type"] == "terminal.ready"
    assert ready_payload["session_id"] == "session-browser"
