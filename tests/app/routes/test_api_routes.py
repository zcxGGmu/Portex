...
def test_openapi_schema_documents_route_and_schema_details(api_client: TestClient) -> None:
    ...
    export_terminal_history_operation = schema["paths"]["/terminals/{group_id}/sessions/history/export"]["get"]
    assert "export" in export_terminal_history_operation["summary"].lower()
    assert "404" in export_terminal_history_operation["responses"]
    export_parameter_names = {item["name"] for item in export_terminal_history_operation["parameters"]}
    assert "limit" in export_parameter_names
    assert "offset" in export_parameter_names
    assert "status" in export_parameter_names
    assert "owner_user_id" in export_parameter_names
    assert "session_id_prefix" in export_parameter_names
    ...
    assert "sort" in search_parameter_names

    export_terminal_history_search_operation = schema["paths"]["/terminals/{group_id}/sessions/history/search/export"][
        "get"
    ]
    assert "export" in export_terminal_history_search_operation["summary"].lower()
    assert "404" in export_terminal_history_search_operation["responses"]
    search_export_parameter_names = {
        item["name"] for item in export_terminal_history_search_operation["parameters"]
    }
    assert "q" in search_export_parameter_names
    assert "limit" in search_export_parameter_names
    assert "offset" in search_export_parameter_names
    assert "status" in search_export_parameter_names
    assert "owner_user_id" in search_export_parameter_names
    assert "session_id_prefix" in search_export_parameter_names
    assert "snapshot_from" in search_export_parameter_names
    ...
    assert "detail" in get_terminal_history_detail_operation["summary"].lower()
    assert "404" in get_terminal_history_detail_operation["responses"]

    download_terminal_history_detail_operation = schema["paths"][
    ...
    force_delete_terminal_session_operation = schema["paths"]["/terminals/{group_id}/sessions/force"]["delete"]
    assert "force" in force_delete_terminal_session_operation["summary"].lower()
    assert "404" in force_delete_terminal_session_operation["responses"]

    list_terminal_overview_operation = schema["paths"]["/terminals"]["get"]
    ...
    export_terminal_latest_histories_operation = schema["paths"]["/terminals/history/export"]["get"]
    ...
    list_skills_operation = schema["paths"]["/skills"]["get"]
    assert "skill" in list_skills_operation["summary"].lower()
    assert "401" in list_skills_operation["responses"]

    get_skill_operation = schema["paths"]["/skills/{skill_id}"]["get"]
    assert "skill" in get_skill_operation["summary"].lower()
    assert "404" in get_skill_operation["responses"]

    put_skill_operation = schema["paths"]["/skills/{skill_id}"]["put"]
    assert "update" in put_skill_operation["summary"].lower()
    assert "400" in put_skill_operation["responses"]

    patch_skill_state_operation = schema["paths"]["/skills/{skill_id}/state"]["patch"]
    ...
    assert "SkillSummaryResponse" in schema["components"]["schemas"]
    assert "SkillDetailResponse" in schema["components"]["schemas"]
    ...
    assert "snapshot_at" in terminal_history_summary_schema["properties"]
    ...
    assert "output" in terminal_history_detail_schema["properties"]
    assert "truncated" in terminal_history_detail_schema["properties"]
