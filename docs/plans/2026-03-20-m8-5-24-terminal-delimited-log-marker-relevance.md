# M8.5.24 Terminal Delimited Log-Marker Relevance Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so delimiter-bounded raw line-start log-marker hits such as `query: text` and strict `query - text` outrank glued marker forms such as `query:text` and `query -text`, while preserving the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the existing raw log-marker signal and already-matched offsets to derive one additional delimiter-bounded raw-marker signal family from the character immediately after the marker separator, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Delimited Raw-Marker Ordering

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-20-m8-5-24-terminal-delimited-log-marker-relevance-design.md`
...
- when neither snapshot has a delimited raw-marker hit, ordering falls back to the existing `M8.5.23` exact-tag and wrapper signals
...
- FAIL because current `relevance` does not distinguish delimiter-bounded raw marker hits from glued raw marker hits
...
### Task 2: Implement Minimal Delimited Raw-Marker Relevance Signals
...
Use a local helper that evaluates already-matched offsets as delimited raw-marker hits only when they already satisfy the current line-start raw log-marker rule and the marker separator is followed by either end-of-output or whitespace.
...
- PASS for the new delimited raw-marker-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.24 delimited log-marker relevance"
```
...
git commit -m "fix(terminal): stabilize M8.5.24 delimited log-marker regressions"
```
...
- `M8.5.24` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.24` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.24 delimited log-marker relevance context"
```
