# M8.5.29 Terminal Non-Square-Bracket Exact-Tag Colon-Marker Priority Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so non-square-bracket exact-tag colon-marker hits such as `(query): text`, `{query}: text`, and `<query>: text` outrank non-square-bracket exact-tag dash-marker hits such as `(query) - text`, while preserving the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the existing exact-tag colon-marker, exact-tag, and square-bracket exact-tag signals to derive one additional non-square-bracket colon-marker signal family from already matched offsets, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Non-Square-Bracket Colon-Marker Ordering

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-20-m8-5-29-terminal-non-square-bracket-exact-tag-colon-marker-priority-design.md`

**Step 1: Write the failing test**

Add focused service tests that assert:

- non-square-bracket exact-tag colon-marker hits such as `(query): text` outrank non-square-bracket exact-tag dash-marker hits such as `(query) - text`
- when non-square-bracket colon-marker counts tie, earlier `first_line_start_non_square_bracket_exact_tag_colon_marker_offset` wins
- when neither snapshot has a non-square-bracket exact-tag colon-marker hit, ordering falls back to the existing `M8.5.28` chain
...
- FAIL because current `relevance` does not distinguish non-square-bracket exact-tag colon markers from non-square-bracket exact-tag dash markers
...
### Task 2: Implement Minimal Non-Square-Bracket Colon-Marker Signals
...
Use a local helper that evaluates already-matched offsets as non-square-bracket exact-tag colon-marker hits only when they already satisfy the current line-start exact-tag colon-marker rule, already satisfy the current line-start exact-tag rule, and explicitly do not satisfy the current line-start square-bracket exact-tag rule.
...
- PASS for the new non-square-bracket-colon-marker-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.29 non-square colon-marker priority"
```
...
git commit -m "fix(terminal): stabilize M8.5.29 non-square colon-marker regressions"
```
...
- `M8.5.29` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.29` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.29 non-square colon-marker context"
```
