# M8.5.28 Terminal Square-Bracket Exact-Tag Dash-Marker Priority Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so square-bracket exact-tag dash-marker hits such as `[query] - text` outrank other wrapper exact-tag dash-marker hits such as `(query) - text`, while preserving the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the existing exact-tag marker, square-bracket exact-tag, and already-matched offsets to derive one additional dash-marker signal family from the exact characters immediately after the closing wrapper, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Square-Bracket Dash-Marker Ordering

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-20-m8-5-28-terminal-square-bracket-exact-tag-dash-marker-priority-design.md`

**Step 1: Write the failing test**

Add focused service tests that assert:

- square-bracket exact-tag dash-marker hits such as `[query] - text` outrank other wrapper exact-tag dash-marker hits such as `(query) - text`
- when square-bracket dash-marker counts tie, earlier `first_line_start_square_bracket_exact_tag_dash_marker_offset` wins
- when neither snapshot has a square-bracket exact-tag dash-marker hit, ordering falls back to the existing `M8.5.27` chain
...
- FAIL because current `relevance` does not distinguish square-bracket exact-tag dash markers from other wrapper exact-tag dash markers
...
### Task 2: Implement Minimal Square-Bracket Dash-Marker Signals
...
Use a local helper that evaluates already-matched offsets as square-bracket exact-tag dash-marker hits only when they already satisfy the current line-start exact-tag marker rule, already satisfy the current line-start square-bracket exact-tag rule, and the exact characters immediately after the closing wrapper are the strict delimiter ` -`, followed by end-of-output or whitespace.
...
- PASS for the new square-bracket-dash-marker-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.28 square-bracket dash-marker priority"
```
...
git commit -m "fix(terminal): stabilize M8.5.28 dash-marker regressions"
```
...
- `M8.5.28` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.28` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.28 square-bracket dash-marker context"
```
