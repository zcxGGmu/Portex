# M8.5.26 Terminal Square-Bracket Exact-Tag Priority Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so square-bracket exact-tag hits such as `[query] text`, `[query]: text`, and `[query] - text` outrank other wrapper exact-tag hits such as `(query) text`, while preserving the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the existing exact-tag wrapper signal and already-matched offsets to derive one additional square-bracket signal family from the wrapper pair itself, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Square-Bracket Exact-Tag Priority

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-20-m8-5-26-terminal-square-bracket-exact-tag-priority-design.md`
...
- when neither snapshot has a square-bracket exact-tag hit, ordering falls back to the existing `M8.5.25` chain
...
- FAIL because current `relevance` does not distinguish square-bracket exact-tag hits from other exact-tag wrapper hits
...
### Task 2: Implement Minimal Square-Bracket Exact-Tag Signals
...
Use a local helper that evaluates already-matched offsets as square-bracket exact-tag hits only when they already satisfy the current line-start exact-tag rule and the wrapper pair is exactly `[ ]`.
...
- PASS for the new square-bracket-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.26 square-bracket exact-tag priority"
```
...
git commit -m "fix(terminal): stabilize M8.5.26 square-bracket exact-tag regressions"
```
...
- `M8.5.26` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.26` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.26 square-bracket exact-tag priority context"
```
