# M8.5.23 Terminal Exact-Tag Line-Start Relevance Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so delimiter-aware line-start exact-tag wrapper hits such as `[query] text`, `[query]: text`, and `[query] - text` outrank tighter non-delimited wrapper hits such as `[query]text`, while preserving the stronger `M8.5.21` raw marker priority and the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the existing punctuation-wrapper signal and already-matched offsets to derive one additional exact-tag signal family from the character(s) immediately after the closing wrapper, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Exact-Tag Wrapper Ordering

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-20-m8-5-23-terminal-exact-tag-line-start-relevance-design.md`

**Step 1: Write the failing test**

Add focused service tests that assert:

- delimiter-aware exact-tag wrapper hits such as `[query] text` outrank tighter non-delimited wrapper hits such as `[query]text`
- when exact-tag counts tie, earlier `first_line_start_exact_tag_offset` wins
- when neither snapshot has an exact-tag hit, ordering falls back to the existing `M8.5.22` punctuation-wrapper signals
...
- FAIL because current `relevance` does not distinguish delimiter-aware exact-tag wrapper hits from tighter non-delimited wrapper hits
...
### Task 2: Implement Minimal Exact-Tag Wrapper Relevance Signals
...
Use a local helper that evaluates already-matched offsets as exact-tag hits only when they already satisfy the current line-start punctuation-wrapper rule and the character immediately after the closing wrapper is one of:
...
- PASS for the new exact-tag-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.23 exact-tag line-start relevance"
```
...
git commit -m "fix(terminal): stabilize M8.5.23 exact-tag regressions"
```
...
- `M8.5.23` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.23` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.23 exact-tag line-start relevance context"
```
