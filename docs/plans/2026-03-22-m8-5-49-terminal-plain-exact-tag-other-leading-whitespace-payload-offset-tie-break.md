# M8.5.49 Terminal Plain Exact-Tag Other-Leading-Whitespace Payload Offset Tie-Break Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a narrow backend-only `relevance` refinement so other-leading-whitespace payload plain exact-tag separators that appear later outrank equally noisy snapshots where those separators appear earlier.

**Architecture:** Keep route/API/frontend contracts unchanged and only extend `TerminalSessionService` internal search-candidate metadata. Add one conditional earliest other-leading-whitespace payload offset derived from the residual-family predicate, inject it into the `relevance` tuple immediately after the `M8.5.48` mixed-whitespace-payload-offset key, and preserve compatibility boundaries.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Other-Leading-Whitespace Payload Offset Tie-Break

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-22-m8-5-49-terminal-plain-exact-tag-other-leading-whitespace-payload-offset-tie-break-design.md`

**Step 1: Write failing tests**

Add focused tests that assert:

- when stronger signals tie, later other-leading-whitespace payload separator offset ranks ahead
...
```bash
.venv/bin/pytest tests/services/test_terminal_sessions.py -k "other_leading_whitespace_payload_offset_tie_break or prefers_later_other_leading_whitespace_payload" -q
```
...
### Task 2: Implement Minimal Other-Leading-Whitespace Payload Offset Metadata And Sort Key
...
**Step 2: Add other-leading-whitespace payload offset helper logic**
...
- return sentinel when no residual-family hit exists

**Step 3: Wire `_build_search_candidate(...)`**

Compute conditional residual-family offset:

- when `line_start_plain_exact_tag_single_space_separator_match_count > 0`, use earliest residual-family offset
...
```bash
git add services/terminal_sessions.py tests/services/test_terminal_sessions.py docs/plans/2026-03-22-m8-5-49-terminal-plain-exact-tag-other-leading-whitespace-payload-offset-tie-break-design.md docs/plans/2026-03-22-m8-5-49-terminal-plain-exact-tag-other-leading-whitespace-payload-offset-tie-break.md
git commit -m "feat(terminal): add M8.5.49 other leading whitespace payload offset tie-break"
```
...
git commit -m "docs(handoff): sync M8.5.49 other leading whitespace payload offset context"
```
