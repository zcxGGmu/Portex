# M8.5.27 Terminal Exact-Tag Colon-Marker Relevance Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so exact-tag colon-marker hits such as `[query]: text` outrank exact-tag dash-marker hits such as `[query] - text`, while preserving the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the existing exact-tag marker signal and already-matched offsets to derive one additional colon-marker signal family from the exact character immediately after the closing wrapper, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Exact-Tag Colon-Marker Ordering

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-20-m8-5-27-terminal-exact-tag-colon-marker-relevance-design.md`
...
- when neither snapshot has an exact-tag colon-marker hit, ordering falls back to the existing `M8.5.26` chain
...
- FAIL because current `relevance` does not distinguish exact-tag colon markers from exact-tag dash markers
...
### Task 2: Implement Minimal Exact-Tag Colon-Marker Signals
...
Use a local helper that evaluates already-matched offsets as exact-tag colon-marker hits only when they already satisfy the current line-start exact-tag marker rule and the exact character immediately after the closing wrapper is `:`, followed by end-of-output or whitespace.
...
- PASS for the new exact-tag-colon-marker-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.27 exact-tag colon-marker relevance"
```
...
git commit -m "fix(terminal): stabilize M8.5.27 exact-tag colon-marker regressions"
```
...
- `M8.5.27` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.27` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.27 exact-tag colon-marker relevance context"
```
