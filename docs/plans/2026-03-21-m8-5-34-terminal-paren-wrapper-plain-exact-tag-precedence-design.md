# M8.5.34 Terminal Paren-Wrapper Plain Exact-Tag Precedence Design

## Goal

Improve the default `relevance` ordering for `/terminals` history search so paren-wrapped plain exact-tag hits such as `(query) text` outrank other non-square plain exact-tag hits such as `{query} text` and `<query> text`, while preserving the existing priority of raw line-start markers, exact-tag marker families, square-bracket wrapper preference, and all current API/UI/history compatibility boundaries.
...
- keep the existing match set unchanged:
  ...
  - the new paren-wrapper plain exact-tag signal only improves ordering among already matched snapshots
...
- preserve the existing `M8.5.33` relevance signals around the new signal:
...
- no broader wrapper-family model beyond the narrow `()` plain exact-tag preference
- no changes to marker-family precedence from `M8.5.25` through `M8.5.33`
...
`M8.5.22` introduced punctuation-aware line-start wrapper relevance. `M8.5.23` then refined delimiter-aware exact-tag wrappers such as `[query] text`, `[query]: text`, and `[query] - text`. `M8.5.25` through `M8.5.32` iteratively refined the marker branch of the exact-tag family: exact-tag markers, square-bracket priority, colon-marker and dash-marker sub-ordering, then `()` over `{}` / `<>`, and finally `{}` over `<>` for wrapper markers. `M8.5.33` then refined the plain exact-tag branch by demoting `<query> text` behind the other non-square plain exact-tag wrappers.

The next remaining rough edge is now the unresolved tie between `(error) failed` and `{error} failed` in the plain exact-tag branch. The smallest next step is to mirror the existing marker-branch preference and reward the `()` plain exact-tag family over `{}` while leaving the broader search contract unchanged.

## Approaches Considered

### 1. Narrow paren-wrapper plain exact-tag preference

Add a signal that rewards only paren-wrapped plain exact-tag hits.

Pros:

- smallest implementation delta
- aligns the plain exact-tag branch with the existing marker-branch direction
...
- also breaks the current tie

Cons:

- conflicts with the established marker-branch ordering
- harder to justify as a coherent search policy
...
Use approach 1: a narrow paren-wrapper plain exact-tag preference.

This keeps the change additive and consistent with the existing marker branch. It improves only the `()` slice of the plain exact-tag family without broadening the public search contract or forcing a full wrapper-family ordering model.

## Paren-Wrapper Plain Exact-Tag Model
...
The new signal is a narrow preference inside the plain exact-tag branch. It does not change the broader raw-marker-versus-wrapper hierarchy, does not change any exact-tag marker ordering, and works together with the existing `<>` demotion from `M8.5.33`.
...
- under `sort="relevance"`, paren-wrapped plain exact-tag results simply rank ahead of brace-wrapped plain exact-tag results when stronger signals are otherwise similar
...
15. `line_start_punctuation_wrap_match_count` descending
16. `line_start_whole_word_match_count` descending
17. `conditional_non_line_start_whole_word_match_count` ascending
18. `whole_word_match_count` descending
19. `first_line_start_log_marker_offset` ascending
20. `first_line_start_delimited_log_marker_offset` ascending
21. `first_line_start_exact_tag_colon_marker_offset` ascending
22. `first_line_start_square_bracket_exact_tag_dash_marker_offset` ascending
23. `first_line_start_paren_wrapper_marker_offset` ascending
24. `first_line_start_brace_wrapper_marker_offset` ascending
25. `first_line_start_non_square_bracket_exact_tag_colon_marker_offset` ascending
26. `first_line_start_non_square_bracket_exact_tag_dash_marker_offset` ascending
27. `first_line_start_paren_wrapper_plain_exact_tag_offset` ascending
28. `first_line_start_exact_tag_marker_offset` ascending
29. `first_line_start_exact_tag_offset` ascending
30. `first_line_start_square_bracket_exact_tag_offset` ascending
31. `first_line_start_punctuation_wrap_offset` ascending
32. `first_line_start_whole_word_offset` ascending
33. `first_whole_word_offset` ascending
34. `cluster_span` ascending
35. `first_match_offset` ascending
36. `match_density` descending
37. `snapshot_at` descending
38. `session_id` ascending
...
- `first_line_start_paren_wrapper_plain_exact_tag_offset`
  - earliest query offset among paren-wrapper plain exact-tag hits
  - when no such hit exists, use a stable sentinel so ordering falls back cleanly

All other metric semantics remain identical to `M8.5.33`.
...
- earlier `first_line_start_paren_wrapper_plain_exact_tag_offset` wins when paren-wrapper plain exact-tag counts tie
- when no paren-wrapper plain exact-tag signal exists, ordering falls back to the `M8.5.33` chain
