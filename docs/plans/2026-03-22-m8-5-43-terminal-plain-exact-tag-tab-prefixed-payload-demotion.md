# M8.5.43 Terminal Plain Exact-Tag Tab-Prefixed Payload Demotion Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a narrow backend-only `relevance` refinement so snapshots with clean single-space plain exact-tag hits prefer fewer tab-prefixed payload-bearing plain exact-tag separators.

**Architecture:** Keep route/API/frontend contracts unchanged and only extend `TerminalSessionService` internal search-candidate metadata. Add one conditional tab-prefixed payload count derived from existing exact-tag helpers, inject it into the `relevance` tuple immediately after the `M8.5.42` payloadless-offset key, and preserve all compatibility boundaries.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Tab-Prefixed Payload Tie-Break

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-22-m8-5-43-terminal-plain-exact-tag-tab-prefixed-payload-demotion-design.md`
...
```bash
.venv/bin/pytest tests/services/test_terminal_sessions.py -k "tab_prefixed_payload or no_single_space_plain_exact_tag" -q
```
...
### Task 2: Implement Minimal Tab-Prefixed Payload Metadata And Sort Key
...
**Step 2: Add tab-prefixed payload helper logic**
...
- exclude existing single-space and payloadless forms
- the first separator character after the closing wrapper must be `\t`
- at least one non-whitespace payload character must appear before newline/end

**Step 3: Wire `_build_search_candidate(...)`**

Compute conditional tab-prefixed payload count:
...
- immediately after `-item.conditional_first_line_start_plain_exact_tag_payloadless_separator_offset`
...
```bash
git add services/terminal_sessions.py tests/services/test_terminal_sessions.py docs/plans/2026-03-22-m8-5-43-terminal-plain-exact-tag-tab-prefixed-payload-demotion-design.md docs/plans/2026-03-22-m8-5-43-terminal-plain-exact-tag-tab-prefixed-payload-demotion.md
git commit -m "feat(terminal): add M8.5.43 tab payload demotion"
```
...
git commit -m "docs(handoff): sync M8.5.43 tab payload context"
```
