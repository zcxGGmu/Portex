# M8.5.34 Terminal Paren-Wrapper Plain Exact-Tag Precedence Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so paren-wrapped plain exact-tag hits such as `(query) text` outrank brace-wrapped plain exact-tag hits such as `{query} text`, while preserving the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the existing exact-tag and exact-tag-marker signals to derive one additional paren-wrapper plain exact-tag signal family from already matched offsets, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Paren-Wrapper Plain Exact-Tag Precedence

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-21-m8-5-34-terminal-paren-wrapper-plain-exact-tag-precedence-design.md`

**Step 1: Write the failing test**

Add focused service tests that assert:

- plain exact-tag hits such as `(query) text` outrank brace-wrapped plain exact-tag hits such as `{query} text`
- when paren-wrapper plain exact-tag counts tie, earlier `first_line_start_paren_wrapper_plain_exact_tag_offset` wins
- when neither snapshot has a paren-wrapper plain exact-tag hit, ordering falls back to the existing `M8.5.33` chain
...
- FAIL because current `relevance` still treats `(query) text` as a peer of `{query} text` in the plain exact-tag branch
...
### Task 2: Implement Minimal Paren-Wrapper Plain Exact-Tag Signals
...
Use a local helper that evaluates already-matched offsets as paren-wrapper plain exact-tag hits only when they already satisfy the current line-start exact-tag rule, explicitly do not satisfy the current line-start exact-tag marker rule, and the exact wrapper pair around the query is `(` and `)`.
...
Update the `relevance` sort tuple to insert the new signal immediately after `line_start_square_bracket_exact_tag_match_count` and before `line_start_angle_wrapper_plain_exact_tag_match_count`:
...
- PASS for the new paren-wrapper-plain-exact-tag-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.34 paren plain exact-tag precedence"
```
...
git commit -m "fix(terminal): stabilize M8.5.34 paren plain exact-tag regressions"
```
...
- `M8.5.34` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.34` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.34 paren plain exact-tag context"
```
