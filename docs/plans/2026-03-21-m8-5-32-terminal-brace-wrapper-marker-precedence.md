# M8.5.32 Terminal Brace-Wrapper Marker Precedence Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so brace-wrapped exact-tag marker hits such as `{query}: text` and `{query} - text` outrank angle-wrapped exact-tag marker hits such as `<query>: text` and `<query> - text`, while preserving the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the existing exact-tag marker signal and already-matched offsets to derive one additional brace-wrapper marker signal family, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Brace-Wrapper Marker Precedence

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-21-m8-5-32-terminal-brace-wrapper-marker-precedence-design.md`

**Step 1: Write the failing test**

Add focused service tests that assert:

- brace-wrapper marker hits such as `{query}: text` and `{query} - text` outrank angle-wrapper marker hits such as `<query>: text` and `<query> - text`
- when brace-wrapper marker counts tie, earlier `first_line_start_brace_wrapper_marker_offset` wins
- when neither snapshot has a brace-wrapper marker hit, ordering falls back to the existing `M8.5.31` chain
...
- FAIL because current `relevance` does not distinguish `{}` wrapper markers from `<>` wrapper markers
...
### Task 2: Implement Minimal Brace-Wrapper Marker Signals
...
Use a local helper that evaluates already-matched offsets as brace-wrapper marker hits only when they already satisfy the current line-start exact-tag marker rule and the exact wrapper pair around the query is `{` and `}`.
...
- PASS for the new brace-wrapper-marker-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.32 brace-wrapper precedence"
```
...
git commit -m "fix(terminal): stabilize M8.5.32 brace-wrapper regressions"
```
...
- `M8.5.32` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.32` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.32 brace-wrapper context"
```
