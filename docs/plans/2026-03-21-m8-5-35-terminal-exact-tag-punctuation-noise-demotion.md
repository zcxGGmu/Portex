# M8.5.35 Terminal Exact-Tag Punctuation-Noise Demotion Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so exact-tag hits such as `(query) text`, `{query} text`, and `[query] text` outrank otherwise similar snapshots that carry extra tighter-wrapper punctuation noise such as `(query)text` or `{query}text`, while preserving the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the already-computed exact-tag and punctuation-wrap counts to derive one additional conditional noise-demotion signal, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Exact-Tag Punctuation-Noise Demotion

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-21-m8-5-35-terminal-exact-tag-punctuation-noise-demotion-design.md`
...
- when neither snapshot has an exact-tag hit, ordering falls back to the existing `M8.5.34` chain
...
- FAIL because current `relevance` still lets extra tighter-wrapper punctuation hits inflate `line_start_punctuation_wrap_match_count` even when an exact-tag hit already exists
...
### Task 2: Implement Minimal Exact-Tag Punctuation-Noise Demotion
...
```python
non_exact_tag_punctuation_wrap_match_count = (
...
No new parser/helper is needed because the signal is strictly derived from existing counts.

**Step 2: Update only the `relevance` sort branch**

Keep `newest` and `oldest` unchanged.

Update the `relevance` sort tuple to insert the new demotion signal immediately after `line_start_angle_wrapper_plain_exact_tag_match_count` and before `line_start_punctuation_wrap_match_count`:
...
- PASS for the new exact-tag-punctuation-noise-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.35 exact-tag punctuation-noise demotion"
```
...
git commit -m "fix(terminal): stabilize M8.5.35 exact-tag punctuation-noise regressions"
```
...
- `M8.5.35` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.35` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.35 exact-tag punctuation-noise context"
```
