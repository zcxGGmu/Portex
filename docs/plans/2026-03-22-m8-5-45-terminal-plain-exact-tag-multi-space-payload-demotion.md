# M8.5.45 Terminal Plain Exact-Tag Multi-Space Payload Demotion Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a narrow backend-only `relevance` refinement so snapshots with clean single-space plain exact-tag hits prefer fewer multi-space payload-bearing plain exact-tag separators.

**Architecture:** Keep route/API/frontend contracts unchanged and only extend `TerminalSessionService` internal search-candidate metadata. Add one conditional multi-space payload count derived from existing exact-tag helpers, inject it into the `relevance` tuple immediately after the `M8.5.44` tab-payload-offset key, and preserve all compatibility boundaries.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Multi-Space Payload Tie-Break

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-22-m8-5-45-terminal-plain-exact-tag-multi-space-payload-demotion-design.md`
...
```bash
.venv/bin/pytest tests/services/test_terminal_sessions.py -k "multi_space_payload or no_single_space_plain_exact_tag" -q
```
...
### Task 2: Implement Minimal Multi-Space Payload Metadata And Sort Key
...
**Step 2: Add multi-space payload helper logic**
...
- immediately after `-item.conditional_first_line_start_plain_exact_tag_tab_prefixed_payload_offset`
...
```bash
git add services/terminal_sessions.py tests/services/test_terminal_sessions.py docs/plans/2026-03-22-m8-5-45-terminal-plain-exact-tag-multi-space-payload-demotion-design.md docs/plans/2026-03-22-m8-5-45-terminal-plain-exact-tag-multi-space-payload-demotion.md
git commit -m "feat(terminal): add M8.5.45 multi-space payload demotion"
```
...
git commit -m "docs(handoff): sync M8.5.45 multi-space payload context"
```
