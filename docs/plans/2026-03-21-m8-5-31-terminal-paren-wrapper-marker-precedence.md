# M8.5.31 Terminal Paren-Wrapper Marker Precedence Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so parenthesis-wrapped exact-tag marker hits such as `(query): text` and `(query) - text` outrank other non-square wrapper marker hits such as `{query}: text`, `<query>: text`, `{query} - text`, and `<query> - text`, while preserving the existing API/UI/history compatibility boundaries.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse the existing exact-tag marker signal and already-matched offsets to derive one additional paren-wrapper marker signal family, then reuse the current search route, pagination, and frontend behavior as regression coverage rather than expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Paren-Wrapper Marker Precedence

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-21-m8-5-31-terminal-paren-wrapper-marker-precedence-design.md`

**Step 1: Write the failing test**

Add focused service tests that assert:

- paren-wrapper marker hits such as `(query): text` and `(query) - text` outrank brace and angle-wrapper marker hits such as `{query}: text`, `<query>: text`, `{query} - text`, and `<query> - text`
- when paren-wrapper marker counts tie, earlier `first_line_start_paren_wrapper_marker_offset` wins
- when neither snapshot has a paren-wrapper marker hit, ordering falls back to the existing `M8.5.30` chain
...
- FAIL because current `relevance` does not distinguish `()` wrapper markers from other non-square wrapper markers
...
### Task 2: Implement Minimal Paren-Wrapper Marker Signals
...
Use a local helper that evaluates already-matched offsets as paren-wrapper marker hits only when they already satisfy the current line-start exact-tag marker rule and the exact wrapper pair around the query is `(` and `)`.
...
- PASS for the new paren-wrapper-marker-focused service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.31 paren-wrapper precedence"
```
...
git commit -m "fix(terminal): stabilize M8.5.31 paren-wrapper regressions"
```
...
- `M8.5.31` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.31` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.31 paren-wrapper context"
```
