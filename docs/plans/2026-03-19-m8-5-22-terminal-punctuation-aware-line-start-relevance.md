# M8.5.22 Terminal Punctuation-Aware Line-Start Relevance Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Refine default terminal-history `relevance` ordering so narrow line-start punctuation-wrapped markers such as `[query]` and `(query)` outrank plain line-start whole-word hits, while preserving the stronger `M8.5.21` preference for line-start `query:` and strict `query -` markers.

**Architecture:** Keep the current terminal history search contract intact and only extend the backend `relevance` candidate metadata inside `TerminalSessionService`. Reuse already-matched offsets to derive one additional punctuation-wrapper signal family, then reuse the existing search route, pagination, and frontend behavior as regression coverage instead of expanding the surface area.

**Tech Stack:** FastAPI, Python dataclasses, pytest, Ruff, React 19, TypeScript, Vite

---

### Task 1: Add Failing Service Tests For Punctuation-Aware Line-Start Ordering

**Files:**
- Modify: `tests/services/test_terminal_sessions.py`
- Reference: `services/terminal_sessions.py`
- Reference: `docs/plans/2026-03-19-m8-5-22-terminal-punctuation-aware-line-start-relevance-design.md`

**Step 1: Write the failing test**

Add focused service tests that assert:

- line-start `query:` and strict `query -` marker results still outrank punctuation-wrapper hits such as `[query]`
- punctuation-wrapper hits such as `[query]` outrank plain line-start whole-word hits when broader strength is otherwise similar
- when punctuation-wrapper counts tie, earlier `first_line_start_punctuation_wrap_offset` wins
- when neither snapshot has a punctuation-wrapper hit, ordering falls back to the existing `M8.5.21` relevance signals
...
- FAIL because current `relevance` does not distinguish `[query]`-style line-start wrappers from plain line-start hits
...
### Task 2: Implement Minimal Punctuation-Aware Line-Start Relevance Signals
...
Use a local helper that evaluates already-matched offsets as wrapper hits only when:

- the query already satisfies the current whole-word rule
- the previous character is one of `[` `(` `{` `<`
- that opening punctuation sits at transcript start or immediately after `\n`
...
- PASS for the new punctuation-aware line-start service coverage

**Step 4: Commit**

```bash
git add tests/services/test_terminal_sessions.py services/terminal_sessions.py
git commit -m "feat(terminal): add M8.5.22 punctuation-aware line-start relevance"
```
...
git commit -m "fix(terminal): stabilize M8.5.22 punctuation-aware regressions"
```
...
- `M8.5.22` scope and behavior
- latest verification evidence
- next suggested post-`M8.5.22` search-quality refinement

**Step 3: Commit**

```bash
git add docs/progress.md AGENTS.md tasks/todo.md
git commit -m "docs(handoff): sync M8.5.22 punctuation-aware line-start context"
```
