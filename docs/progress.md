...
当前分支: `main`
最新功能提交: `1171501` (`feat(terminal): expand offline relevance brace and angle fallback fixtures`)
最新 planning 提交: `38f9385`（`docs(plans): add offline relevance brace and angle fallback expansion plan`）
最近一次 handoff 同步: 当前分支以 `docs(handoff): sync offline relevance brace and angle fallback context` 为最新重启同步入口；如需精确 hash，请使用当前分支 `git log --oneline`
...
- `M8.5.9` 已完成并已落地到 `main`（per-workspace terminal history output search + detail highlighting）。
...
- `M8.5.23` 已完成并已合入当前工作树（`cd2e80e`；补齐 backend-only exact-tag line-start relevance refinement，新增对行首 `[query] text` / `[query]: text` / `[query] - text` 这类 delimiter-aware wrapper 结果的排序优先级，同时保持 `M8.5.21` raw `query:` / strict `query -` marker 仍高于 wrapper 家族，且 route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界不变）。
- `M8.5.24` 已完成并已合入当前工作树（`ab24843`；补齐 backend-only delimited log-marker relevance refinement，新增对行首 `query: text` / strict `query - text` 这类 delimiter-bounded raw marker 结果的排序优先级，同时保持 glued `query:text` / `query -text` 仍属于 raw marker 家族，且 wrapper/exact-tag、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界不变）。
...
- `M8.5.29` 已完成并已合入当前工作树（`16685cd`；补齐 backend-only non-square-bracket exact-tag colon-marker priority refinement，新增对行首 `(query): ...` / `{query}: ...` / `<query>: ...` 这类 non-square-bracket exact-tag colon-marker 结果的排序优先级；只在共享 square-bracket colon-marker 已经把更强信号打平后的 tie-break 中，压过更晚出现的 non-square-bracket colon-marker 排列，同时保持 square-bracket colon-marker 链、raw marker 家族、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界不变）。
- `M8.5.30` 已完成并已合入当前工作树（`e6bf5f2`；补齐 backend-only non-square-bracket exact-tag dash-marker placement refinement，新增对行首 `(query) - ...` / `{query} - ...` / `<query> - ...` 这类 non-square-bracket exact-tag dash-marker 结果的排序优先级；只在共享 square-bracket dash-marker 已经把更强信号打平后的 tie-break 中，压过更晚出现的 non-square-bracket dash-marker 排列，同时保持 square-bracket dash-marker 链、non-square colon-marker 链、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界不变）。
...
- `M8.5.35` 已完成并已合入当前工作树（`67fcc71`；补齐 backend-only exact-tag punctuation-noise demotion refinement，新增对 line-start exact-tag snapshot 内 tighter-wrapper punctuation 噪音的条件性降权；只在存在 exact-tag 命中时把额外的 non-exact-tag punctuation-wrap 计作噪音，同时保持 marker 家族、square-bracket 链、plain wrapper-family 链、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界不变）。
- `M8.5.36` 已完成并已合入当前工作树（`fb4effd`；补齐 backend-only brace-wrapper plain exact-tag precedence refinement，新增对 non-marker exact-tag 分支里 `{query} text` 的显式 count/offset tie-break；只在既有 `M8.5.35` 链中追加 brace plain exact-tag 元数据，不改 route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界）。
...
- `M8.5.41` 已完成并已合入当前工作树（`a5c563f`；补齐 backend-only plain exact-tag payloadless separator demotion refinement，新增对 non-marker exact-tag 分支里“当 single-space plain exact-tag 已存在时，payloadless plain exact-tag separator 越少越好”的显式 tie-break；只在既有 `M8.5.40` 链中追加 conditional payloadless count 元数据，不改 route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界）。
- `M8.5.42` 已完成并已合入当前工作树（`6fce60f`；补齐 backend-only plain exact-tag payloadless separator offset tie-break refinement，新增对 non-marker exact-tag 分支里“当 single-space plain exact-tag 已存在且 payloadless 计数打平时，payloadless separator 更晚出现更好”的显式 tie-break；只在既有 `M8.5.41` 链中追加 conditional payloadless earliest-offset 元数据，不改 route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界）。
- `M8.5.43` 已完成并已合入当前工作树（`faf0ec1`；补齐 backend-only plain exact-tag tab-prefixed payload demotion refinement，新增对 non-marker exact-tag 分支里“当 single-space plain exact-tag 已存在时，tab-prefixed payload separator 越少越好”的显式 tie-break；只在既有 `M8.5.42` 链中追加 conditional tab-prefixed payload count 元数据，不改 route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界）。
- `M8.5.44` 已完成并已合入当前工作树（`61d3382`；补齐 backend-only plain exact-tag tab-prefixed payload offset tie-break refinement，新增对 non-marker exact-tag 分支里“当 single-space plain exact-tag 已存在且 tab-prefixed payload 计数打平时，tab-prefixed payload separator 更晚出现更好”的显式 tie-break；只在既有 `M8.5.43` 链中追加 conditional tab-prefixed payload earliest-offset 元数据，不改 route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界）。
- `M8.5.45` 已完成并已合入当前工作树（`cbf4014`；补齐 backend-only plain exact-tag multi-space payload demotion refinement，新增对 non-marker exact-tag 分支里“当 single-space plain exact-tag 已存在时，multi-space payload separator 越少越好”的显式 tie-break；只在既有 `M8.5.44` 链中追加 conditional multi-space payload count 元数据，不改 route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界）。
- `M8.5.46` 已完成并已合入当前工作树（`cd6fbdf`；补齐 backend-only plain exact-tag multi-space payload offset tie-break refinement，新增对 non-marker exact-tag 分支里“当 single-space plain exact-tag 已存在且 multi-space payload 计数打平时，multi-space payload separator 更晚出现更好”的显式 tie-break；只在既有 `M8.5.45` 链中追加 conditional multi-space payload earliest-offset 元数据，不改 route/UI/snippet/DTO、`latest.json`、`/sessions/current/history` 与 RBAC 边界）。
...
- terminal relevance 离线评估 whitespace-family 扩样已完成并已合入当前工作树（`eeb388e`；`tests/fixtures/terminal_relevance_baseline.json` 从 12 个样本扩到 16 个样本，新增 payloadless separator quality、tab-prefixed payload offset pagination、multi-space payload offset pagination、space-prefixed mixed-whitespace payload offset pagination，`tests/scripts/test_evaluate_terminal_relevance.py` 同步 case 数与 case-id 断言）。
...
- terminal relevance 离线评估 brace/angle fallback 扩样已完成并已合入当前工作树（`1171501`；`tests/fixtures/terminal_relevance_baseline.json` 从 32 个样本扩到 36 个样本，新增 brace plain offset tie-break、angle plain pagination、no-brace-wrapper fallback、no-angle-plain fallback，`tests/scripts/test_evaluate_terminal_relevance.py` 同步 case 数与 case-id 断言）。
- 当前起点：当前功能代码基线为 `M8.5.51` + `fd0344a` + terminal relevance offline baseline（`c4efc33` + `0ba9c2d` + `ab90bbc` + `eeb388e` + `841e3e2` + `150a412` + `76cecc3` + `bd16927` + `1171501`）；terminal 搜索线若继续，优先在当前 36-case 离线基准上继续补充真实/边界样本并观察指标，只有样本或指标暴露稳定缺口时再决定是否继续细分 tie-break；保持 `latest.json`、`/sessions/current/history`、API/UI 与 RBAC 边界不变。正式 `docs/TODO.md` 仍停在 `M6.5.3`。

---

## 2. 最近完成

- terminal relevance offline brace/angle fallback expansion（on `main`）：新增 `docs/plans/2026-03-24-terminal-relevance-offline-baseline-brace-and-angle-fallback-expansion-design.md` 与 `docs/plans/2026-03-24-terminal-relevance-offline-baseline-brace-and-angle-fallback-expansion.md`，范围固定为“继续扩充离线基准，不改排序逻辑/route/API/UI/RBAC/search contract”。
- terminal relevance offline brace/angle fallback expansion（on `main`）：`tests/fixtures/terminal_relevance_baseline.json` 从 32 case 扩到 36 case，新增 brace plain offset tie-break、angle plain pagination、no-brace-wrapper fallback、no-angle-plain fallback。
- terminal relevance offline brace/angle fallback expansion（on `main`）：`tests/scripts/test_evaluate_terminal_relevance.py` 现断言 `case_count=36` 并校验新增 brace/angle fallback case-id 集合，离线脚本继续保持 `pass_rate/top1_accuracy/mrr` 指标与失败返回码契约不变。
- terminal relevance offline brace/angle fallback expansion（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/scripts/test_evaluate_terminal_relevance.py -q`（RED→GREEN）、`.venv/bin/python scripts/evaluate_terminal_relevance.py --format text`（`case_count=36`, `pass_count=36`, `pass_rate/top1_accuracy/mrr = 1.000`）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`740 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint && npm run build` 与 `git diff --check`。
- terminal relevance offline brace/angle fallback expansion（on `main`）：planning 提交为 `38f9385`（`docs(plans): add offline relevance brace and angle fallback expansion plan`）；功能提交为 `1171501`（`feat(terminal): expand offline relevance brace and angle fallback fixtures`）。
...
- terminal relevance offline non-square marker expansion（on `main`）：`tests/scripts/test_evaluate_terminal_relevance.py` 现断言 `case_count=28` 并校验新增 non-square marker case-id 集合，离线脚本继续保持 `pass_rate/top1_accuracy/mrr` 指标与失败返回码契约不变。
- terminal relevance offline non-square marker expansion（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/scripts/test_evaluate_terminal_relevance.py -q`（RED→GREEN）、`.venv/bin/python scripts/evaluate_terminal_relevance.py --format text`（`case_count=28`, `pass_count=28`, `pass_rate/top1_accuracy/mrr = 1.000`）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`740 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint && npm run build` 与 `git diff --check`。
- terminal relevance offline non-square marker expansion（on `main`）：planning 提交为 `c9c6ef1`（`docs(plans): add offline relevance non-square marker expansion plan`）；功能提交为 `76cecc3`（`feat(terminal): expand offline relevance non-square marker fixtures`）。

- terminal relevance offline offset tie-break expansion（on `main`）：新增 `docs/plans/2026-03-24-terminal-relevance-offline-baseline-offset-tie-break-expansion-design.md` 与 `docs/plans/2026-03-24-terminal-relevance-offline-baseline-offset-tie-break-expansion.md`，范围固定为“继续扩充离线基准，不改排序逻辑/route/API/UI/RBAC/search contract”。
- terminal relevance offline offset tie-break expansion（on `main`）：`tests/fixtures/terminal_relevance_baseline.json` 从 20 case 扩到 24 case，新增 exact-tag colon-marker offset tie-break、square-bracket dash-marker offset tie-break、paren plain-wrapper offset tie-break、square-bracket plain exact-tag offset tie-break。
- terminal relevance offline offset tie-break expansion（on `main`）：`tests/scripts/test_evaluate_terminal_relevance.py` 现断言 `case_count=24` 并校验新增 offset-tie-break case-id 集合，离线脚本继续保持 `pass_rate/top1_accuracy/mrr` 指标与失败返回码契约不变。
- terminal relevance offline offset tie-break expansion（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/scripts/test_evaluate_terminal_relevance.py -q`（RED→GREEN）、`.venv/bin/python scripts/evaluate_terminal_relevance.py --format text`（`case_count=24`, `pass_count=24`, `pass_rate/top1_accuracy/mrr = 1.000`）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`740 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint && npm run build` 与 `git diff --check`。
- terminal relevance offline offset tie-break expansion（on `main`）：planning 提交为 `ca0be06`（`docs(plans): add offline relevance offset tie-break expansion plan`）；功能提交为 `150a412`（`feat(terminal): expand offline relevance offset tie-break fixtures`）。

- terminal relevance offline marker/plain-wrapper expansion（on `main`）：新增 `docs/plans/2026-03-24-terminal-relevance-offline-baseline-marker-and-plain-wrapper-expansion-design.md` 与 `docs/plans/2026-03-24-terminal-relevance-offline-baseline-marker-and-plain-wrapper-expansion.md`，范围固定为“继续扩充离线基准，不改排序逻辑/route/API/UI/RBAC/search contract”。
- terminal relevance offline marker/plain-wrapper expansion（on `main`）：`tests/fixtures/terminal_relevance_baseline.json` 从 16 case 扩到 20 case，新增 exact-tag colon-marker pagination、square-bracket dash-marker pagination、square-bracket exact-tag pagination、paren plain-wrapper pagination。
- terminal relevance offline marker/plain-wrapper expansion（on `main`）：`tests/scripts/test_evaluate_terminal_relevance.py` 现断言 `case_count=20` 并校验新增 marker/plain-wrapper case-id 集合，离线脚本继续保持 `pass_rate/top1_accuracy/mrr` 指标与失败返回码契约不变。
- terminal relevance offline marker/plain-wrapper expansion（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/scripts/test_evaluate_terminal_relevance.py -q`（RED→GREEN）、`.venv/bin/python scripts/evaluate_terminal_relevance.py --format text`（`case_count=20`, `pass_count=20`, `pass_rate/top1_accuracy/mrr = 1.000`）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`740 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint && npm run build` 与 `git diff --check`。
- terminal relevance offline marker/plain-wrapper expansion（on `main`）：planning 提交为 `e64f835`（`docs(plans): add offline relevance marker and plain wrapper expansion plan`）；功能提交为 `841e3e2`（`feat(terminal): expand offline relevance marker and plain wrapper fixtures`）。

- terminal relevance offline whitespace-family expansion（on `main`）：新增 `docs/plans/2026-03-24-terminal-relevance-offline-baseline-whitespace-family-expansion-design.md` 与 `docs/plans/2026-03-24-terminal-relevance-offline-baseline-whitespace-family-expansion.md`，范围固定为“继续扩充离线基准，不改排序逻辑/route/API/UI/RBAC/search contract”。
- terminal relevance offline whitespace-family expansion（on `main`）：`tests/fixtures/terminal_relevance_baseline.json` 从 12 case 扩到 16 case，新增 payloadless separator quality、tab-prefixed payload offset pagination、multi-space payload offset pagination、space-prefixed mixed-whitespace payload offset pagination。
- terminal relevance offline whitespace-family expansion（on `main`）：`tests/scripts/test_evaluate_terminal_relevance.py` 现断言 `case_count=16` 并校验新增 whitespace-family case-id 集合，离线脚本继续保持 `pass_rate/top1_accuracy/mrr` 指标与失败返回码契约不变。
- terminal relevance offline whitespace-family expansion（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/scripts/test_evaluate_terminal_relevance.py -q`（RED→GREEN）、`.venv/bin/python scripts/evaluate_terminal_relevance.py --format text`（`case_count=16`, `pass_count=16`, `pass_rate/top1_accuracy/mrr = 1.000`）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`740 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint && npm run build` 与 `git diff --check`。
- terminal relevance offline whitespace-family expansion（on `main`）：planning 提交为 `6226cea`（`docs(plans): add offline relevance whitespace family expansion plan`）；功能提交为 `eeb388e`（`feat(terminal): expand offline relevance whitespace family fixtures`）。
...
- `M8.5.51` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_first_line_start_plain_exact_tag_other_leading_mixed_whitespace_payload_offset` 与对应 sentinel，并新增 mixed-other earliest-offset helper；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、满足 other-leading mixed-whitespace payload 规则的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与降序（更晚更好）排序。该信号放在 `M8.5.50` 的 conditional mixed-other count 之后、`M8.5.49` 的 broad other-leading earliest-offset 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.51` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k "other_leading_mixed_whitespace_payload_offset_tie_break or prefers_later_other_leading_mixed_whitespace_payload" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`737 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.51` implementation（on `main`）：planning 提交为 `684474a`（`docs(plans): add M8.5.51 mixed-other payload offset plan`）；功能提交为 `d569c75`（`feat(terminal): add M8.5.51 mixed-other payload offset tie-break`）。
...
- refined `M8.5.49` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_first_line_start_plain_exact_tag_other_leading_whitespace_payload_offset` 与对应 sentinel，并新增 residual-family earliest-offset helper；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、不是 single-space、不是 payloadless、不是 tab-prefixed payload、不是 multi-space payload、不是 space-prefixed mixed-whitespace payload、closing wrapper 后首个 separator 字符为既非空格也非 tab 的其他 whitespace 且后续仍有非空白 payload 的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与降序（更晚更好）排序。该信号放在 `M8.5.48` 的 conditional mixed-whitespace payload earliest-offset 之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- refined `M8.5.49` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k "other_leading_whitespace_payload_offset_tie_break or prefers_later_other_leading_whitespace_payload" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`731 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- refined `M8.5.49` implementation（on `main`）：feature 提交为 `cac916a`（`feat(terminal): add M8.5.49 other leading whitespace payload offset tie-break`）。

- `M8.5.48` planning（on `main`）：新增 `docs/plans/2026-03-22-m8-5-48-terminal-plain-exact-tag-space-prefixed-mixed-whitespace-payload-offset-tie-break-design.md` 与 `docs/plans/2026-03-22-m8-5-48-terminal-plain-exact-tag-space-prefixed-mixed-whitespace-payload-offset-tie-break.md`，范围固定为“backend-only plain exact-tag space-prefixed mixed-whitespace payload offset tie-break refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.48` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 stronger signals 与 mixed-whitespace payload 计数已打平时“mixed-whitespace payload separator 更晚出现更好”；无 single-space plain exact-tag 时稳定回退到既有 `M8.5.47` 信号；并补齐基于新全局排序的分页切片行为。
- `M8.5.48` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_first_line_start_plain_exact_tag_space_prefixed_mixed_whitespace_payload_offset` 与对应 sentinel，并新增 mixed-whitespace payload earliest-offset helper；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、不是 single-space、不是 payloadless、不是 tab-prefixed payload、不是 multi-space payload、closing wrapper 后首个 separator 字符为 ASCII 空格且第二个 separator 位置为非空格 whitespace 且后续仍有非空白 payload 的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与降序（更晚更好）排序。该信号放在 `M8.5.47` 的 conditional mixed-whitespace payload count 之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.48` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k "mixed_whitespace_payload_offset_tie_break or prefers_later_mixed_whitespace_payload" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`728 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.48` implementation（on `main`）：planning 提交为 `d94f5a0`（`docs(plans): add M8.5.48 mixed whitespace payload offset plan`）；功能提交为 `fc77eca`（`feat(terminal): add M8.5.48 mixed whitespace payload offset tie-break`）。

- `M8.5.47` planning（on `main`）：新增 `docs/plans/2026-03-22-m8-5-47-terminal-plain-exact-tag-space-prefixed-mixed-whitespace-payload-demotion-design.md` 与 `docs/plans/2026-03-22-m8-5-47-terminal-plain-exact-tag-space-prefixed-mixed-whitespace-payload-demotion.md`，范围固定为“backend-only plain exact-tag space-prefixed mixed-whitespace payload demotion refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.47` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 stronger signals 已打平时更少的 space-prefixed mixed-whitespace payload plain exact-tag separator 高于更多 mixed-whitespace payload separator、无 single-space plain exact-tag 时稳定回退到既有 `M8.5.46` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.47` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_line_start_plain_exact_tag_space_prefixed_mixed_whitespace_payload_match_count` 与 mixed-whitespace payload helper/count；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、不是 single-space、不是 payloadless、不是 tab-prefixed payload、不是 multi-space payload、closing wrapper 后首个 separator 字符为 ASCII 空格且第二个 separator 位置为非空格 whitespace 且后续仍有非空白 payload 的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与升序（更少更好）排序。该信号放在 `M8.5.46` 的 conditional multi-space payload earliest-offset 之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.47` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k "mixed_whitespace_payload" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`725 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.47` implementation（on `main`）：planning 提交为 `e5cf837`（`docs(plans): add M8.5.47 mixed whitespace payload plan`）；功能提交为 `b3b90f1`（`feat(terminal): add M8.5.47 mixed whitespace payload demotion`）。

- `M8.5.46` planning（on `main`）：新增 `docs/plans/2026-03-22-m8-5-46-terminal-plain-exact-tag-multi-space-payload-offset-tie-break-design.md` 与 `docs/plans/2026-03-22-m8-5-46-terminal-plain-exact-tag-multi-space-payload-offset-tie-break.md`，范围固定为“backend-only plain exact-tag multi-space payload offset tie-break refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.46` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 stronger signals 与 multi-space payload 计数已打平时“multi-space payload separator 更晚出现更好”；无 single-space plain exact-tag 时稳定回退到既有 `M8.5.45` 信号；并补齐基于新全局排序的分页切片行为。
- `M8.5.46` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_first_line_start_plain_exact_tag_multi_space_payload_offset` 与对应 sentinel，并新增 multi-space payload earliest-offset helper；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、不是 single-space、不是 payloadless、不是 tab-prefixed payload、closing wrapper 后前两个 separator 均为 ASCII 空格且后续仍有非空白 payload 的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与降序（更晚更好）排序。该信号放在 `M8.5.45` 的 conditional multi-space payload count 之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.46` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k "multi_space_payload_offset_tie_break or prefers_later_multi_space_payload" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`722 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.46` implementation（on `main`）：planning 提交为 `50bbacd`（`docs(plans): add M8.5.46 multi-space payload offset plan`）；功能提交为 `cd6fbdf`（`feat(terminal): add M8.5.46 multi-space payload offset tie-break`）。

- `M8.5.45` planning（on `main`）：新增 `docs/plans/2026-03-22-m8-5-45-terminal-plain-exact-tag-multi-space-payload-demotion-design.md` 与 `docs/plans/2026-03-22-m8-5-45-terminal-plain-exact-tag-multi-space-payload-demotion.md`，范围固定为“backend-only plain exact-tag multi-space payload demotion refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.45` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 stronger signals 已打平时更少的 multi-space payload plain exact-tag separator 高于更多 multi-space payload separator、无 single-space plain exact-tag 时稳定回退到既有 `M8.5.44` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.45` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_line_start_plain_exact_tag_multi_space_payload_match_count` 与 multi-space payload helper/count；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、不是 single-space、不是 payloadless、不是 tab-prefixed payload、closing wrapper 后前两个 separator 均为 ASCII 空格且后续仍有非空白 payload 的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与升序（更少更好）排序。该信号放在 `M8.5.44` 的 conditional tab-prefixed payload earliest-offset 之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.45` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k "multi_space_payload" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`719 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.45` implementation（on `main`）：planning 提交为 `3ed3b70`（`docs(plans): add M8.5.45 multi-space payload plan`）；功能提交为 `cbf4014`（`feat(terminal): add M8.5.45 multi-space payload demotion`）。

- `M8.5.44` planning（on `main`）：新增 `docs/plans/2026-03-22-m8-5-44-terminal-plain-exact-tag-tab-prefixed-payload-offset-tie-break-design.md` 与 `docs/plans/2026-03-22-m8-5-44-terminal-plain-exact-tag-tab-prefixed-payload-offset-tie-break.md`，范围固定为“backend-only plain exact-tag tab-prefixed payload offset tie-break refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.44` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 stronger signals 与 tab-prefixed payload 计数已打平时“tab-prefixed payload separator 更晚出现更好”；无 single-space plain exact-tag 时稳定回退到既有 `M8.5.43` 信号；并补齐基于新全局排序的分页切片行为。
- `M8.5.44` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_first_line_start_plain_exact_tag_tab_prefixed_payload_offset` 与对应 sentinel，并新增 tab-prefixed payload earliest-offset helper；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、不是 payloadless、closing wrapper 后首个 separator 字符为 tab 且后续仍有非空白 payload 的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与降序（更晚更好）排序。该信号放在 `M8.5.43` 的 conditional tab-prefixed payload count 之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.44` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k "tab_prefixed_payload_offset_tie_break or prefers_later_tab_prefixed_payload" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`716 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.44` implementation（on `main`）：planning 提交为 `906768f`（`docs(plans): add M8.5.44 tab payload offset plan`）；功能提交为 `61d3382`（`feat(terminal): add M8.5.44 tab payload offset tie-break`）。

- `M8.5.43` planning（on `main`）：新增 `docs/plans/2026-03-22-m8-5-43-terminal-plain-exact-tag-tab-prefixed-payload-demotion-design.md` 与 `docs/plans/2026-03-22-m8-5-43-terminal-plain-exact-tag-tab-prefixed-payload-demotion.md`，范围固定为“backend-only plain exact-tag tab-prefixed payload demotion refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.43` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 stronger signals 已打平时更少的 tab-prefixed payload plain exact-tag separator 高于更多 tab-prefixed payload separator、无 single-space plain exact-tag 时稳定回退到既有 `M8.5.42` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.43` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_line_start_plain_exact_tag_tab_prefixed_payload_match_count` 与 tab-prefixed payload helper/count；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、不是 payloadless、closing wrapper 后首个 separator 字符为 tab 且后续仍有非空白 payload 的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与升序（更少更好）排序。该信号放在 `M8.5.42` 的 conditional payloadless earliest-offset 之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.43` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k "tab_prefixed_payload" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`713 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.43` implementation（on `main`）：planning 提交为 `6db793a`（`docs(plans): add M8.5.43 tab payload plan`）；功能提交为 `faf0ec1`（`feat(terminal): add M8.5.43 tab payload demotion`）。

- `M8.5.42` planning（on `main`）：新增 `docs/plans/2026-03-22-m8-5-42-terminal-plain-exact-tag-payloadless-separator-offset-tie-break-design.md` 与 `docs/plans/2026-03-22-m8-5-42-terminal-plain-exact-tag-payloadless-separator-offset-tie-break.md`，范围固定为“backend-only plain exact-tag payloadless separator offset tie-break refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.42` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 stronger signals 与 payloadless 计数已打平时“payloadless separator 更晚出现更好”；无 single-space plain exact-tag 时稳定回退到既有 `M8.5.41` 信号；并补齐基于新全局排序的分页切片行为。
- `M8.5.42` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_first_line_start_plain_exact_tag_payloadless_separator_offset` 与对应 sentinel，并新增 payloadless earliest-offset helper；只认已经满足 line-start exact-tag 且不满足 exact-tag marker 的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与降序（更晚更好）排序。该信号放在 `M8.5.41` 的 conditional payloadless count 之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.42` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k \"payloadless_offset_tie_break or prefers_later_payloadless_plain_exact_tag_separator_offsets\" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`710 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.42` implementation（on `main`）：功能提交为 `6fce60f`（`feat(terminal): add M8.5.42 payloadless offset tie-break`）。

- `M8.5.41` planning（on `main`）：新增 `docs/plans/2026-03-22-m8-5-41-terminal-plain-exact-tag-payloadless-separator-demotion-design.md` 与 `docs/plans/2026-03-22-m8-5-41-terminal-plain-exact-tag-payloadless-separator-demotion.md`，范围固定为“backend-only plain exact-tag payloadless separator demotion refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.41` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定在 stronger signals 已打平时，payloadless plain exact-tag separator 更少的结果优先；无 single-space plain exact-tag 时稳定回退到既有 `M8.5.40` 信号；并补齐基于新全局排序的分页切片行为。
- `M8.5.41` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_line_start_plain_exact_tag_payloadless_separator_match_count`，并新增 payloadless helper/count；只认已经满足 line-start exact-tag 且不满足 exact-tag marker 的窄范围结果，且只在存在 single-space plain exact-tag 命中时参与升序（更少更好）排序。该信号放在 `M8.5.40` 的 conditional earliest separator-noise offset 之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 之前。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.41` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -k \"payloadless_plain_exact_tag or no_single_space_plain_exact_tag_exists\" -q`（RED→GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`707 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.41` implementation（on `main`）：功能提交为 `a5c563f`（`feat(terminal): add M8.5.41 payloadless separator demotion`）。
...
- `M8.5.39` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 single-space plain exact-tag 高于 tab/更松散空白分隔的 plain exact-tag、无 single-space plain exact-tag 时稳定回退到既有 `M8.5.38` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.39` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_plain_exact_tag_single_space_separator_match_count` / `first_line_start_plain_exact_tag_single_space_separator_offset` 与对应 sentinel，并新增 narrow plain exact-tag 单空格分隔 helper/count；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、closing wrapper 后紧跟单个 ASCII 空格且随后立刻进入非空白文本的窄范围结果。该信号放在 plain wrapper-specific count/offset 链之后、`conditional_non_exact_tag_punctuation_wrap_match_count` 与 generic exact-tag marker offset 之前参与排序。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.39` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`701 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.39` implementation（on `main`）：功能提交为 `3378c02`（`feat(terminal): add M8.5.39 plain exact-tag single-space preference`）。

- `M8.5.38` planning（on `main`）：新增 `docs/plans/2026-03-21-m8-5-38-terminal-square-bracket-plain-exact-tag-offset-tie-break-design.md` 与 `docs/plans/2026-03-21-m8-5-38-terminal-square-bracket-plain-exact-tag-offset-tie-break.md`，将范围固定为“backend-only square-bracket plain exact-tag offset tie-break refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.38` planning（on `main`）：规划提交为 `b0667a8`（`docs(plans): add M8.5.38 square-bracket plain exact-tag offset plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.38` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 plain square-bracket exact-tag offset tie-break、无 plain square-bracket exact-tag 时稳定回退到既有 `M8.5.37` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.38` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `first_line_start_square_bracket_plain_exact_tag_offset` 与对应 sentinel，并新增 plain square-bracket exact-tag helper/count；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、同时 wrapper pair 恰好为 `[]` 的窄范围结果。该信号放在 non-square dash-marker offset 之后、paren plain exact-tag offset 之前，并按偏移升序参与排序。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.38` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`698 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.38` implementation（on `main`）：功能提交为 `ce7f93c`（`feat(terminal): add M8.5.38 square-bracket plain exact-tag offset tie-break`）。

- `M8.5.37` planning（on `main`）：新增 `docs/plans/2026-03-21-m8-5-37-terminal-angle-wrapper-plain-exact-tag-offset-tie-break-design.md` 与 `docs/plans/2026-03-21-m8-5-37-terminal-angle-wrapper-plain-exact-tag-offset-tie-break.md`，将范围固定为“backend-only angle-wrapper plain exact-tag offset tie-break refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.37` planning（on `main`）：规划提交为 `dc1775f`（`docs(plans): add M8.5.37 angle plain exact-tag offset plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.37` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 angle plain exact-tag offset tie-break、无 angle plain exact-tag 时稳定回退到既有 `M8.5.36` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.37` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `first_line_start_angle_wrapper_plain_exact_tag_offset` 与对应 sentinel，并把现有 angle-wrapper plain exact-tag count helper 改为返回 count/first-offset；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、同时 wrapper pair 恰好为 `<>` 的窄范围结果。该信号放在 brace plain exact-tag offset 之后、generic exact-tag marker offset 之前，并按偏移升序参与排序。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.37` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`695 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.37` implementation（on `main`）：功能提交为 `4ff848c`（`feat(terminal): add M8.5.37 angle plain exact-tag offset tie-break`）。

- `M8.5.36` planning（on `main`）：新增 `docs/plans/2026-03-21-m8-5-36-terminal-brace-wrapper-plain-exact-tag-precedence-design.md` 与 `docs/plans/2026-03-21-m8-5-36-terminal-brace-wrapper-plain-exact-tag-precedence.md`，将范围固定为“backend-only brace-wrapper plain exact-tag precedence refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.36` planning（on `main`）：规划提交为 `2e90c03`（`docs(plans): add M8.5.36 brace plain exact-tag plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.36` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 brace plain exact-tag offset tie-break、无 brace plain exact-tag 时稳定回退到既有 `M8.5.35` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.36` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_brace_wrapper_plain_exact_tag_match_count` / `first_line_start_brace_wrapper_plain_exact_tag_offset` 与本地 brace-wrapper plain exact-tag helper；只认已经满足 line-start exact-tag 且不满足 exact-tag marker、同时 wrapper pair 恰好为 `{}` 的窄范围结果。该信号放在 paren plain exact-tag 之后、angle plain exact-tag demotion 之前，并按降序 + 偏移升序参与排序。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.36` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`692 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.36` implementation（on `main`）：功能提交为 `fb4effd`（`feat(terminal): add M8.5.36 brace plain exact-tag precedence`）。

- `M8.5.35` planning（on `main`）：新增 `docs/plans/2026-03-21-m8-5-35-terminal-exact-tag-punctuation-noise-demotion-design.md` 与 `docs/plans/2026-03-21-m8-5-35-terminal-exact-tag-punctuation-noise-demotion.md`，将范围固定为“backend-only exact-tag punctuation-noise demotion refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.35` planning（on `main`）：规划提交为 `2829cdf`（`docs(plans): add M8.5.35 exact-tag punctuation-noise plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.35` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 clean exact-tag 高于带 tighter-wrapper 噪音的 exact-tag、较少 `conditional_non_exact_tag_punctuation_wrap_match_count` 高于较多噪音、无 exact-tag 时稳定回退到既有 `M8.5.34` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.35` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `conditional_non_exact_tag_punctuation_wrap_match_count`；只在存在 line-start exact-tag 命中时，把 `line_start_punctuation_wrap_match_count - line_start_exact_tag_match_count` 作为条件性噪音计数并按升序参与排序，专门修复 exact-tag snapshot 会被额外 tighter-wrapper punctuation 命中意外加分的缺口。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.35` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`689 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.35` implementation（on `main`）：功能提交为 `67fcc71`（`feat(terminal): add M8.5.35 exact-tag punctuation-noise demotion`）。
...
- `M8.5.33` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 `{query} text` 与 `(query) text` 高于 `<query> text`、无 angle-wrapper plain exact-tag 时稳定回退到既有 `M8.5.32` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.33` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_angle_wrapper_plain_exact_tag_match_count` 与本地 angle-wrapper plain exact-tag helper；只认已经满足 line-start exact-tag、同时不满足 exact-tag marker 且 wrapper pair 恰好为 `<>` 的窄范围结果。该信号放在 square-bracket exact-tag 链之后、punctuation wrapper 链之前，并按升序参与排序，专门修复 non-marker exact-tag 分支里 `<>` 应落后于 `()` / `{}` 的缺口。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.33` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`681 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.33` implementation（on `main`）：功能提交为 `ad8e426`（`feat(terminal): add M8.5.33 angle-wrapper exact-tag demotion`）。

- `M8.5.32` planning（on `main`）：新增 `docs/plans/2026-03-21-m8-5-32-terminal-brace-wrapper-marker-precedence-design.md` 与 `docs/plans/2026-03-21-m8-5-32-terminal-brace-wrapper-marker-precedence.md`，将范围固定为“backend-only brace-wrapper marker precedence refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.32` planning（on `main`）：规划提交为 `ec19e37`（`docs(plans): add M8.5.32 brace-wrapper precedence plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.32` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 brace-wrapper marker 高于 angle-wrapper marker、brace-wrapper offset tie-break、无 brace-wrapper marker 时稳定回退到既有 `M8.5.31` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.32` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_brace_wrapper_marker_match_count` / `first_line_start_brace_wrapper_marker_offset` 与本地 brace-wrapper helper；只认已经满足 `M8.5.25` exact-tag marker 条件且 wrapper pair 恰好为 `{}` 的窄范围结果。该信号放在 paren-wrapper 链之后、generic non-square marker 链之前，专门修复 non-square wrapper marker 家族里 `{}` 应先于 `<>` 的缺口。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.32` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`677 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.32` implementation（on `main`）：功能提交为 `7888b84`（`feat(terminal): add M8.5.32 brace-wrapper precedence`）。

- `M8.5.31` planning（on `main`）：新增 `docs/plans/2026-03-21-m8-5-31-terminal-paren-wrapper-marker-precedence-design.md` 与 `docs/plans/2026-03-21-m8-5-31-terminal-paren-wrapper-marker-precedence.md`，将范围固定为“backend-only paren-wrapper marker precedence refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.31` planning（on `main`）：规划提交为 `06730cd`（`docs(plans): add M8.5.31 paren-wrapper precedence plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.31` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 paren-wrapper marker 高于 brace/angle-wrapper marker、paren-wrapper offset tie-break、无 paren-wrapper marker 时稳定回退到既有 `M8.5.30` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.31` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_paren_wrapper_marker_match_count` / `first_line_start_paren_wrapper_marker_offset` 与本地 paren-wrapper helper；只认已经满足 `M8.5.25` exact-tag marker 条件且 wrapper pair 恰好为 `()` 的窄范围结果。该信号放在 square-bracket 相关链之后、generic non-square marker 链之前，专门修复 non-square wrapper marker 家族里 `()` 应先于 `{}` / `<>` 的缺口。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.31` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`673 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.31` implementation（on `main`）：功能提交为 `7dc9940`（`feat(terminal): add M8.5.31 paren-wrapper precedence`）。

- `M8.5.30` planning（on `main`）：新增 `docs/plans/2026-03-21-m8-5-30-terminal-non-square-bracket-exact-tag-dash-marker-placement-design.md` 与 `docs/plans/2026-03-21-m8-5-30-terminal-non-square-bracket-exact-tag-dash-marker-placement.md`，将范围固定为“backend-only non-square-bracket exact-tag dash-marker placement refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.30` planning（on `main`）：规划提交为 `1872a93`（`docs(plans): add M8.5.30 non-square dash-marker placement plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.30` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定在共享 square-bracket dash-marker 已经把更强信号打平后，更早的 non-square-bracket dash-marker 排列优于更晚的 non-square-bracket dash-marker 排列、non-square-bracket dash-marker offset tie-break、无 non-square-bracket dash-marker 时稳定回退到既有 `M8.5.29` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.30` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_non_square_bracket_exact_tag_dash_marker_match_count` / `first_line_start_non_square_bracket_exact_tag_dash_marker_offset` 与本地 dash-marker helper；只认已经满足 `M8.5.25` exact-tag marker 条件、同时不满足 `M8.5.26` square-bracket exact-tag 条件且 wrapper 后紧跟 strict ` -` + delimiter 的窄范围结果。该信号放在 non-square colon-marker 之后、generic exact-tag 之前，专门修复共享 square-bracket dash-marker 打平后，non-square-bracket dash-marker 的更早出现位置仍应优先的缺口。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.30` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`669 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.30` implementation（on `main`）：功能提交为 `e6bf5f2`（`feat(terminal): add M8.5.30 non-square dash-marker placement`）。

- `M8.5.29` planning（on `main`）：新增 `docs/plans/2026-03-20-m8-5-29-terminal-non-square-bracket-exact-tag-colon-marker-priority-design.md` 与 `docs/plans/2026-03-20-m8-5-29-terminal-non-square-bracket-exact-tag-colon-marker-priority.md`，将范围固定为“backend-only non-square-bracket exact-tag colon-marker priority refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.29` planning（on `main`）：规划提交为 `ed891bb`（`docs(plans): add M8.5.29 non-square colon-marker priority plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.29` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定在共享 square-bracket colon-marker 已经把更强信号打平后，更早的 non-square-bracket colon-marker 排列优于更晚的 non-square-bracket dash/colon 排列、non-square-bracket colon-marker offset tie-break、无 non-square-bracket colon-marker 时稳定回退到既有 `M8.5.28` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.29` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_non_square_bracket_exact_tag_colon_marker_match_count` / `first_line_start_non_square_bracket_exact_tag_colon_marker_offset` 与本地 colon-marker helper；只认已经满足 `M8.5.27` exact-tag colon-marker 条件、同时不满足 `M8.5.26` square-bracket exact-tag 条件的窄范围结果。该信号放在 square-bracket dash-marker 之后、generic exact-tag 之前，专门修复共享 square-bracket colon-marker 打平后，non-square-bracket colon-marker 的更早出现位置仍应优先的缺口。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.29` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`665 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.29` implementation（on `main`）：功能提交为 `16685cd`（`feat(terminal): add M8.5.29 non-square colon-marker priority`）。
...
- `M8.5.27` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 colon-style exact-tag marker 高于 dash-style exact-tag marker、colon-marker tie-break、无 colon-marker 时稳定回退到既有 `M8.5.26` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.27` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_exact_tag_colon_marker_match_count` / `first_line_start_exact_tag_colon_marker_offset` 与本地 colon-marker helper；只认已经满足 `M8.5.25` exact-tag marker 条件，且 wrapper 后紧跟 `:` + delimiter 的窄范围结果。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.27` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`（RED -> GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`657 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.27` implementation（on `main`）：功能提交为 `cbde3f1`（`feat(terminal): add M8.5.27 exact-tag colon-marker relevance`）。

- `M8.5.26` planning（on `main`）：新增 `docs/plans/2026-03-20-m8-5-26-terminal-square-bracket-exact-tag-priority-design.md` 与 `docs/plans/2026-03-20-m8-5-26-terminal-square-bracket-exact-tag-priority.md`，将范围固定为“backend-only square-bracket exact-tag priority refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.26` planning（on `main`）：规划提交为 `0a22370`（`docs(plans): add M8.5.26 square-bracket exact-tag priority plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.26` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 square-bracket exact-tag 高于其他 wrapper exact-tag、square-bracket tie-break、无 square-bracket exact-tag 时稳定回退到既有 `M8.5.25` 信号，以及基于新全局排序的分页切片行为。
- `M8.5.26` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_square_bracket_exact_tag_match_count` / `first_line_start_square_bracket_exact_tag_offset` 与本地 square-bracket helper；只认已经满足 `M8.5.23` exact-tag 条件，且 wrapper pair 恰好是 `[ ]` 的窄范围结果。raw marker 家族仍整体高于 wrapper 家族。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.26` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`（RED -> GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`653 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.26` implementation（on `main`）：功能提交为 `17ab845`（`feat(terminal): add M8.5.26 square-bracket exact-tag priority`）。

- `M8.5.25` planning（on `main`）：新增 `docs/plans/2026-03-20-m8-5-25-terminal-exact-tag-marker-relevance-design.md` 与 `docs/plans/2026-03-20-m8-5-25-terminal-exact-tag-marker-relevance.md`，将范围固定为“backend-only exact-tag marker relevance refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.25` planning（on `main`）：规划提交为 `724d34b`（`docs(plans): add M8.5.25 exact-tag marker relevance plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.25` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 exact-tag marker 高于 plain exact-tag、exact-tag marker tie-break、无 exact-tag marker 时稳定回退到既有 `M8.5.24` raw-marker + exact-tag 信号，以及基于新全局排序的分页切片行为。
- `M8.5.25` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_exact_tag_marker_match_count` / `first_line_start_exact_tag_marker_offset` 与本地 exact-tag marker helper；只认已经满足 `M8.5.23` exact-tag 条件，且 closing wrapper 后紧跟 `:` + delimiter 或 strict ` -` + delimiter 的窄范围结果。raw marker 家族仍整体高于 wrapper 家族。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.25` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`（RED -> GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`649 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.25` implementation（on `main`）：功能提交为 `8dd1ad2`（`feat(terminal): add M8.5.25 exact-tag marker relevance`）。

- `M8.5.24` planning（on `main`）：新增 `docs/plans/2026-03-20-m8-5-24-terminal-delimited-log-marker-relevance-design.md` 与 `docs/plans/2026-03-20-m8-5-24-terminal-delimited-log-marker-relevance.md`，将范围固定为“backend-only delimited log-marker relevance refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.24` planning（on `main`）：规划提交为 `fb141e4`（`docs(plans): add M8.5.24 delimited log-marker relevance plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.24` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 delimiter-bounded raw marker 高于 glued raw marker、delimited raw-marker tie-break、无 delimited raw marker 时稳定回退到既有 `M8.5.23` exact-tag / wrapper 信号，以及基于新全局排序的分页切片行为。
- `M8.5.24` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_delimited_log_marker_match_count` / `first_line_start_delimited_log_marker_offset` 与本地 delimited raw-marker helper；只认已经满足 `M8.5.21` line-start raw marker 条件，且 marker separator 后紧跟 end-of-output 或 whitespace 的窄范围结果。raw marker 家族仍整体高于 wrapper 家族。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.24` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`（RED -> GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`645 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.24` implementation（on `main`）：功能提交为 `ab24843`（`feat(terminal): add M8.5.24 delimited log-marker relevance`）。

- `M8.5.23` planning（on `main`）：新增 `docs/plans/2026-03-20-m8-5-23-terminal-exact-tag-line-start-relevance-design.md` 与 `docs/plans/2026-03-20-m8-5-23-terminal-exact-tag-line-start-relevance.md`，将范围固定为“backend-only exact-tag line-start relevance refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.23` planning（on `main`）：规划提交为 `940eac2`（`docs(plans): add M8.5.23 exact-tag line-start relevance plan`）；该提交包含 design + implementation plan 两份新文档。
- `M8.5.23` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 delimiter-aware exact-tag wrapper 高于 tighter wrapper、exact-tag tie-break、无 exact-tag 时稳定回退到既有 `M8.5.22` punctuation-wrapper 信号，以及基于新全局排序的分页切片行为。
- `M8.5.23` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_exact_tag_match_count` / `first_line_start_exact_tag_offset` 与本地 exact-tag helper；只认已经满足 `M8.5.22` line-start wrapper 条件，且 closing wrapper 后紧跟 end-of-output、whitespace、`:` + delimiter、或 strict ` -` + delimiter 的窄范围 exact-tag 结果。既有 `M8.5.21` raw `query:` / strict `query -` marker 仍保持更高优先级。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.23` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`（RED -> GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`641 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.23` implementation（on `main`）：功能提交为 `cd2e80e`（`feat(terminal): add M8.5.23 exact-tag line-start relevance`）。
...
- `M8.5.21` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 line-start `query:` / strict `query -` marker 优先、marker tie-break，以及基于新全局排序的分页切片行为。
- `M8.5.21` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增 `line_start_log_marker_match_count` / `first_line_start_log_marker_offset` 与本地 marker helper，只认 line-start whole-word 命中后紧跟 `:` 或 strict ` -` 的窄范围 marker。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.21` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`（RED -> GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`633 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.21` implementation（on `main`）：功能提交为 `bf40eca`（`feat(terminal): add M8.5.21 log-marker relevance`）。

- `M8.5.20` planning（on `main`）：新增 `docs/plans/2026-03-19-m8-5-20-terminal-line-start-quality-relevance-design.md` 与 `docs/plans/2026-03-19-m8-5-20-terminal-line-start-quality-relevance.md`，将范围固定为“backend-only line-start quality relevance refinement”，明确只扩展 `TerminalSessionService` 内部 `relevance` 排序元数据，不改 `sort` API、route/UI、`latest.json`、`/sessions/current/history` 或 RBAC。
- `M8.5.20` planning（on `main`）：初始规划提交为 `b06b00b`（`docs(plans): add M8.5.20 line-start quality relevance plan`），随后通过 `ac31ffe`（`docs(plans): refine M8.5.20 line-start quality ordering`）与 `a167ffa`（`docs(plans): clarify M8.5.20 conditional noise fallback`）修正两处关键设计点：`whole_word_match_count` 不能压过新噪音信号，且噪音信号必须在无 line-start 命中时保持中性回退。
- `M8.5.20` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`，新增 focused TDD 覆盖，锁定 cleaner line-start 结果优先、噪音 tie 后仍按首个 line-start 偏移排序，以及基于新全局排序的分页切片行为。
- `M8.5.20` implementation（on `main`）：扩展 `services/terminal_sessions.py` 的 `relevance` candidate metadata，新增条件性 `conditional_non_line_start_whole_word_match_count`，仅在存在 line-start whole-word 命中时参与排序；无 line-start 命中时保持中性回退到既有 `M8.5.19` 信号。`newest` / `oldest`、route/UI/snippet/DTO、`latest.json`、`/sessions/current/history`、RBAC 均保持不变。
- `M8.5.20` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`（RED -> GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`629 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
- `M8.5.20` implementation（on `main`）：功能提交为 `ef60f28`（`feat(terminal): add M8.5.20 line-start quality relevance`）。
...
- `M8.5.18` implementation（on `main`）：修复 no-whole-word fallback 稳定性，使用稳定 sentinel 避免长度相关的排序泄漏，并强化 fallback regression fixture。
- `M8.5.18` implementation（on `main`）：功能提交为 `244da16`（`feat(terminal): add M8.5.18 word-boundary relevance`）与 `8a122d2`（`fix(terminal): stabilize M8.5.18 no-whole-word fallback ordering`）；当前 `main` 还包含 `3c47abc`、`107a213`、`7e8614d` 三个 handoff 同步提交。
- `M8.5.18` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`622 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
...
- `M8.5.17` implementation（on `feat/m8-5-17-terminal-relevance-ranking`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py -q`（RED -> GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`618 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
...
- `M8.5.16` implementation（on `main`）：扩展 `web/src/api/client.ts`、`web/src/hooks/useApi.ts` 与 `web/src/pages/Terminals.tsx`，为 `/terminals` search form 增加排序选择器；切换排序时重置 `searchOffset` / `pendingSearchPageMove` / `pendingMatchTarget` 与 detail anchor，`Clear` 会恢复 `relevance`。
- `M8.5.16` implementation（on `main`）：里程碑实现提交为 `5761a3a`（`feat(terminal): add M8.5.16 search sort controls`）。
- `M8.5.16` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`614 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。

- `M8.5.15` implementation（on `main`）：新增 `docs/plans/2026-03-17-m8-5-15-terminal-preset-time-ranges-design.md` 与 `docs/plans/2026-03-17-m8-5-15-terminal-preset-time-ranges.md`，将范围固定为“frontend-only preset time-range shortcuts”，明确不新增 backend query 参数、不改变 `latest.json` / `/sessions/current/history` 契约。
- `M8.5.15` implementation（on `main`）：扩展 `web/src/pages/Terminals.tsx`，新增 `1h` / `6h` / `24h` / `7d` / `30d` preset 按钮，将快捷范围写回既有 `snapshotFromLocal` / `snapshotToLocal`，timeline 与 search 继续共享同一时间过滤状态。
- `M8.5.15` implementation（on `main`）：新增 `activePresetId` 前端状态；切换 preset 时复用既有 timeline/search/detail reset 逻辑，手动修改 `Snapshot From` / `Snapshot To` 时清空 preset 选中态，避免 UI 与请求状态分叉。
- `M8.5.15` implementation（on `main`）：里程碑实现提交为 `b52697f`（`feat(web): add M8.5.15 terminal preset time ranges`），并已通过 `a3fb321`（`docs(handoff): sync M8.5.15 preset time-range context`）同步到当前 `main` handoff。
- `M8.5.15` implementation（on `main`）：fresh 验证已通过 `cd web && npm run build`（frontend RED：preset helpers/type 未定义）-> `cd web && npm run build`（GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`611 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。

- `M8.5.14` implementation（on `main`）：新增 `docs/plans/2026-03-17-m8-5-14-terminal-time-range-filters-design.md` 与 `docs/plans/2026-03-17-m8-5-14-terminal-time-range-filters.md`，将范围固定为“timeline/search time-range filters”，明确保持 `latest.json` 与 `/sessions/current/history` 兼容。
- `M8.5.14` implementation（on `main`）：扩展 `TerminalSessionService` 与 terminal routes，为 timeline/search 同步增加包含边界的 `snapshot_from` / `snapshot_to` 过滤，并在 `snapshot_from > snapshot_to` 时返回 `400`。
- `M8.5.14` implementation（on `main`）：扩展 `/terminals` 前端 filter state，新增本地 minute-granularity `datetime-local` 控件，并在 timeline/search 请求前统一转换为 UTC 参数。
- `M8.5.14` implementation（on `main`）：里程碑实现拆分为 `7515207`（`feat(terminal): add M8.5.14 backend time-range filters`）和 `a628cc9`（`feat(web): add M8.5.14 time-range filter controls`）。
- `M8.5.14` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
...
- `M8.5.11` implementation（on `main`）：扩展 `TerminalSessionService`，新增 `search_history_by_group()` 与搜索读模型（`match_count` + snippets + `total/has_more` 分页），复用 merged snapshots（`in-memory + latest.json + archived snapshots`）进行 workspace 内大小写不敏感子串匹配。
- `M8.5.11` implementation（on `main`）：扩展 terminal schema + route，新增 `TerminalSessionHistorySearchMatchResponse` / `TerminalSessionHistorySearchResponse` 与 `GET /terminals/{group_id}/sessions/history/search?q=...`，保持既有 timeline/detail/current-history 路由与 RBAC 映射不变。
- `M8.5.11` implementation（on `main`）：扩展 `/terminals` 搜索与导航 UX，新增搜索结果分页表格、detail 输出命中高亮、以及上一条/下一条命中跨 session 与跨搜索页边界跳转能力。
- `M8.5.11` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_api_routes.py -q`（red->green）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`602 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
...
- `M8.5.8` implementation（on `feat/m8-5-8-terminal-history-filters-detail`）：扩展 terminal schema + route，给 `TerminalSessionHistorySummaryResponse` 增加 `snapshot_at`，新增 `TerminalSessionHistoryDetailResponse` 与 `GET /terminals/{group_id}/sessions/history/{session_id}`，同时为 timeline route 增加过滤 query 参数并保持既有 access/error mapping。
- `M8.5.8` implementation（on `feat/m8-5-8-terminal-history-filters-detail`）：扩展前端 terminal API/hooks 与 `/terminals` 页面，新增 timeline filter controls、`snapshot_at` 列、按条目 `View Details` 动作和同页 history detail panel，不改变 overview 轮询主路径。
- `M8.5.8` implementation（on `feat/m8-5-8-terminal-history-filters-detail`）：扩展 `tests/services/test_terminal_sessions.py`、`tests/app/routes/test_terminal_routes.py` 与 `tests/app/routes/test_api_routes.py`，新增 red-green 覆盖：timeline filters、detail lookup（in-memory/latest/archive）、detail route 404 映射、OpenAPI filter/detail 契约；并补一处 overview summary `snapshot_at` 兼容回退，避免 legacy fake summaries 在 `/terminals` 转换层崩溃。
- `M8.5.8` implementation（on `feat/m8-5-8-terminal-history-filters-detail`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`596 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
...
- `M8.5.6` implementation（on `main`）：扩展 terminal overview schema 与 route：新增 `TerminalSessionHistorySummaryResponse`，并在 `/terminals` 的每个 workspace 项目上增加 additive `history` 字段。
- `M8.5.6` implementation（on `main`）：扩展 `/terminals` 前端页面，新增 history session/history bytes/history truncated 列，支持 operator 在 overview 直接查看 persisted-history inventory 信号。
- `M8.5.6` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`、`tests/app/routes/test_terminal_monitor_routes.py` 与 `tests/app/routes/test_api_routes.py`，新增 red-green 覆盖：inventory merge（in-memory + persisted fallback）、route history 字段映射与 OpenAPI schema 合约。
- `M8.5.6` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`582 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
...
- `M8.5.3` implementation（on `main`）：扩展 `tests/services/test_terminal_sessions.py`、`tests/app/routes/test_terminal_routes.py` 与 `tests/app/routes/test_api_routes.py` 覆盖 history snapshot、route auth/success/404 与 OpenAPI route contract。
- `M8.5.3` implementation（on `main`）：fresh 验证已通过 `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`574 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
...
- `M8.5.23` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.22` 基础上优先 delimiter-aware exact-tag wrapper 结果，例如 `[query] text`、`[query]: text`、`[query] - text`，并保持 `M8.5.21` raw `query:` 与 strict `query -` marker 仍高于 wrapper 家族；无 exact-tag 命中时稳定回退到既有 `M8.5.22` 信号。
- `M8.5.24` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.23` 基础上优先 delimiter-bounded raw marker 结果，例如 `query: text` 与 strict `query - text`，并保持 glued `query:text` / `query -text` 仍属于 raw marker 家族且整体高于 wrapper 家族；无 delimited raw marker 命中时稳定回退到既有 `M8.5.23` 信号。
...
- `M8.5.29` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.28` 基础上优先 non-square-bracket exact-tag colon-marker 结果，例如 `(query): text`，但仅在共享 square-bracket colon-marker 已经把更强信号打平后的 tie-break 中生效；它专门用于压过更晚出现的 non-square-bracket colon-marker 排列，且无 non-square-bracket colon-marker 命中时稳定回退到既有 `M8.5.28` 信号。
- `M8.5.30` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.29` 基础上优先 non-square-bracket exact-tag dash-marker 结果，例如 `(query) - text`，但仅在共享 square-bracket dash-marker 已经把更强信号打平后的 tie-break 中生效；它专门用于压过更晚出现的 non-square-bracket dash-marker 排列，且无 non-square-bracket dash-marker 命中时稳定回退到既有 `M8.5.29` 信号。
...
- `M8.5.35` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.34` 基础上降低 exact-tag snapshot 内的 tighter-wrapper punctuation 噪音，但仅在存在 exact-tag 命中时生效；它专门用于防止 `{query}text` / `(query)text` 这类 tighter-wrapper 噪音把已经命中的 exact-tag snapshot 意外往前推，且无 exact-tag 命中时稳定回退到既有 `M8.5.34` 信号。
- `M8.5.36` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.35` 基础上显式引入 brace-wrapper plain exact-tag count/offset tie-break（`{query} text`），并在无 brace plain exact-tag 命中路径稳定回退到既有 `M8.5.35` 信号。
...
- `M8.5.41` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.40` 基础上显式引入 conditional payloadless plain exact-tag separator count tie-break；当 single-space plain exact-tag 已存在时，payloadless separator 更少的结果更好，并在无 single-space plain exact-tag 命中路径稳定回退到既有 `M8.5.40` 信号。
- `M8.5.42` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.41` 基础上显式引入 conditional payloadless plain exact-tag separator earliest-offset tie-break；当 single-space plain exact-tag 已存在且 payloadless 计数打平时，payloadless separator 更晚出现的结果更好，并在无 single-space plain exact-tag 命中路径稳定回退到既有 `M8.5.41` 信号。
- `M8.5.43` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.42` 基础上显式引入 conditional tab-prefixed payload plain exact-tag count tie-break；当 single-space plain exact-tag 已存在时，tab-prefixed payload separator 更少的结果更好，并在无 single-space plain exact-tag 命中路径稳定回退到既有 `M8.5.42` 信号。
- `M8.5.44` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.43` 基础上显式引入 conditional tab-prefixed payload plain exact-tag earliest-offset tie-break；当 single-space plain exact-tag 已存在且 tab-prefixed payload 计数打平时，tab-prefixed payload separator 更晚出现的结果更好，并在无 single-space plain exact-tag 命中路径稳定回退到既有 `M8.5.43` 信号。
- `M8.5.45` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.44` 基础上显式引入 conditional multi-space payload plain exact-tag count tie-break；当 single-space plain exact-tag 已存在时，multi-space payload separator 更少的结果更好，并在无 single-space plain exact-tag 命中路径稳定回退到既有 `M8.5.44` 信号。
- `M8.5.46` 当前已合入工作树：默认 `relevance` 现会在既有 `M8.5.45` 基础上显式引入 conditional multi-space payload plain exact-tag earliest-offset tie-break；当 single-space plain exact-tag 已存在且 multi-space payload 计数打平时，multi-space payload separator 更晚出现的结果更好，并在无 single-space plain exact-tag 命中路径稳定回退到既有 `M8.5.45` 信号。
...
- terminal relevance 离线评估基准当前已合入工作树：可通过 `scripts/evaluate_terminal_relevance.py` 在固定 fixture 上离线输出 `pass_rate/top1_accuracy/mrr`，当前 baseline 为 `36/36` 全通过，现已覆盖 raw/wrapper/plain ladders、marker-family pagination、marker/plain-wrapper offset tie-break、non-square marker branch samples、brace/angle branch samples、brace/angle fallback samples、separator quality、payloadless/tab/multi-space/space-prefixed mixed families、`M8.5.49`/`M8.5.50`/`M8.5.51` whitespace 分支与多种 pagination 切片样本。
...
2. 当前功能代码基线已包含 `38f9385` + `1171501` 的 terminal relevance offline brace/angle fallback expansion、`c7fc16e` + `bd16927` 的 brace/angle branch expansion、`c9c6ef1` + `76cecc3` 的 non-square marker expansion、`ca0be06` + `150a412` 的 offset tie-break expansion、`e64f835` + `841e3e2` 的 marker/plain-wrapper expansion、`6226cea` + `eeb388e` 的 whitespace-family expansion、`7c1c4c7` + `ab90bbc` 的 realistic-edge expansion、`d94877d` + `0ba9c2d` 的 offline baseline expansion、`1bce130` + `c4efc33` 的 baseline 初版、`684474a` + `d569c75` 的 `M8.5.51` 排序优化，以及 `fd0344a` 的 auth/message warning fix；关键文件是 `scripts/evaluate_terminal_relevance.py`、`tests/fixtures/terminal_relevance_baseline.json`、`tests/scripts/test_evaluate_terminal_relevance.py`、`services/terminal_sessions.py`、`tests/services/test_terminal_sessions.py`、`pyproject.toml`、`services/auth.py`、`services/message_service.py`。
3. 如继续当前 terminal 开发，优先在当前 36-case 离线 baseline 上继续补充真实/边界样本并观察 `pass_rate/top1_accuracy/mrr`；当前已覆盖 raw/wrapper/plain ladders、marker-family pagination、marker/plain-wrapper offset tie-break、non-square marker branch samples、brace/angle branch samples、brace/angle fallback samples、separator quality、payloadless/tab/multi-space/space-prefixed mixed families、`M8.5.49`/`M8.5.50`/`M8.5.51` whitespace 分支与多种 pagination slice。下一自然入口可优先补仍未进入离线基准的 brace/angle 直接 pairwise ordering samples 与更早阶段少量 remaining fallback gaps；只有样本或指标暴露稳定缺口时再决定是否进入 post-`M8.5.51` 新 tie-break；保持 `latest.json` 与 `/sessions/current/history` 兼容、不扩展权限边界、不引入全文索引、不改 frontend 协议。
4. 复现当前基线建议命令：
...
> 当前工作树已包含 `M8.5.51` terminal relevance 链、`fd0344a` auth/message warning fix、`greenlet` 依赖声明修复，以及 `1171501` 的 36-case 离线 relevance brace/angle fallback baseline 扩样，并已通过全量回归；下一自然入口仍是在该基线上继续补样并用指标驱动是否进入下一轮 tie-break。
