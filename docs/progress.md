...
当前分支: `main`
最新 planning-only 提交: `8a72a0b` (`docs(plans): add M8.5.71 terminal timeline empty-state colspan fix plan`)
最新功能状态: 当前工作树已完成 `M8.5.71` terminal timeline empty-state colspan fix
...
- 最新可执行功能状态已推进到 `M8.5.71`：在保持 `M8.5.70` overview `History Session` 列绑定修正不变的前提下，`/terminals` workspace timeline table 现在把空态行 `colSpan` 修正为与当前 8 列表头一致，不再留下空态表格对齐错误。这个增量仍然只影响前端 timeline 渲染，不改变任何 backend contract。
- `M8.5.71` 继续保持 additive 边界：
  ...
  - 不改 `M8.5.61` 的 page-level `actionKey` / `actionError` / `actionNotice` 模型，只修正 `/terminals` timeline table 的一处空态对齐错误
...
- `docs/plans/2026-04-14-m8-5-71-terminal-timeline-empty-state-colspan-fix-design.md`
- `docs/plans/2026-04-14-m8-5-71-terminal-timeline-empty-state-colspan-fix.md`

当前 terminal 线额外注意点：

- `M8.5.71` 不新增任何顶层 archive 过滤参数，也不改 timeline payload；只是修正一处已有 timeline 空态渲染
...
2. 如继续 terminal 线，先区分两类工作：
   ...
   - operator surface：当前已经具备 overview 导出、latest-history bundle、带 `group_name_prefix`/`group_id_prefix`/`chat_accessible`/`status`/owner/session/time 过滤、逐项清除 chips、一键 reset、archive-only 时间快捷范围、正确的 overview `History Session` 列、以及正确的 timeline 空态列宽的 cross-workspace history archive、detail 导出、timeline current-page bulk JSON 导出、search current-page JSON 导出、timeline all-pages archive JSON 导出、search all-pages archive JSON 导出
...
> `M8.5.71` 已在保持 grouped archive、RBAC、filename、backend/API contracts 和 workspace-scoped history contract 不变的前提下，修正 `/terminals` timeline 空态行的列宽对齐，同时保留 `M8.5.70` 的 overview `History Session` 正确绑定和 `M8.5.69` 的 archive-only 时间快捷范围；terminal relevance baseline 仍保持收敛暂停，除非出现新的真实证据。
