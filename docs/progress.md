...
当前分支: `main`
最新功能提交: `7888b84` (`feat(terminal): add M8.5.32 brace-wrapper precedence`)
最新 planning 提交: `ec19e37` (`docs(plans): add M8.5.32 brace-wrapper precedence plan`)
最近一次 handoff 同步: 当前分支已完成 `M8.5.32` 的 planning + feature 链，本次 docs sync 已补齐 restart 记录；精确提交顺序以当前分支 `git log` 为准
...
- 当前起点：当前功能代码基线已到 `M8.5.32`；若继续 terminal 搜索优化，优先保持 backend-only 范围评估下一类窄范围 relevance 质量信号，例如 non-marker wrapper-family tie-break 或其他同级 marker-quality 规则，不改 `latest.json`、`/sessions/current/history`、API/UI 或 RBAC 边界。正式 `docs/TODO.md` 仍停在 `M6.5.3`。
...
2. 当前功能代码基线已包含 `ec19e37` + `7888b84` 的 `M8.5.32` backend-only brace-wrapper marker precedence refinement；关键文件是 `docs/plans/2026-03-21-m8-5-32-terminal-brace-wrapper-marker-precedence-design.md`、`docs/plans/2026-03-21-m8-5-32-terminal-brace-wrapper-marker-precedence.md`、`services/terminal_sessions.py`、`tests/services/test_terminal_sessions.py`，以及既有 terminal route/API focused suites。
3. 如继续当前 terminal 开发，优先做 post-`M8.5.32` 的小范围 backend-only relevance 质量优化，例如 non-marker wrapper-family tie-break、另一类 wrapper-marker quality 规则，或其他同级窄范围排序信号；保持 `latest.json` 与 `/sessions/current/history` 兼容、不扩展权限边界、不引入全文索引、不改 frontend 协议。当前 `M8.5.32` 只额外优先 brace-wrapper marker 结果，不包含更宽的 wrapper-family weighting 或 tokenizer 规则。
4. 复现当前基线建议命令：
   - `M8.5.32 terminal focused baseline`：`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`
...
> 当前工作树已包含 `M8.5.32` 的 backend-only brace-wrapper marker 优化并通过全量回归；下一自然入口是保持兼容边界不变的另一类窄范围 relevance 微调。
