...
当前分支: `main`
最新功能提交: `61d3382` (`feat(terminal): add M8.5.44 tab payload offset tie-break`)
最新 planning 提交: `906768f`（`docs(plans): add M8.5.44 tab payload offset plan`）
最近一次 handoff 同步: `1dd6191`（`docs(handoff): sync M8.5.43 tab payload context`）；当前分支已完成 `M8.5.44` 的 planning + feature 链，当前 handoff 以本次 `docs/progress.md` 更新为准，精确提交顺序以当前分支 `git log` 为准
...
- 当前起点：当前功能代码基线已到 `M8.5.44`；若继续 terminal 搜索优化，优先保持 backend-only 范围评估下一类窄范围 non-marker exact-tag 质量信号（例如 payloadful 非单空格分隔里的另一类 cleanliness/tie-break 规则，如 multi-space payload family），不改 `latest.json`、`/sessions/current/history`、API/UI 或 RBAC 边界。正式 `docs/TODO.md` 仍停在 `M6.5.3`。
...
2. 当前功能代码基线已包含 `906768f` + `61d3382` 的 `M8.5.44` backend-only plain exact-tag tab-prefixed payload offset tie-break refinement；关键文件是 `docs/plans/2026-03-22-m8-5-44-terminal-plain-exact-tag-tab-prefixed-payload-offset-tie-break-design.md`、`docs/plans/2026-03-22-m8-5-44-terminal-plain-exact-tag-tab-prefixed-payload-offset-tie-break.md`、`services/terminal_sessions.py`、`tests/services/test_terminal_sessions.py`，以及既有 terminal route/API focused suites。
3. 如继续当前 terminal 开发，优先做 post-`M8.5.44` 的小范围 backend-only relevance 质量优化（仍限于 non-marker exact-tag 分支同级 cleanliness/tie-break）；保持 `latest.json` 与 `/sessions/current/history` 兼容、不扩展权限边界、不引入全文索引、不改 frontend 协议。当前 `M8.5.44` 只补齐 conditional tab-prefixed payload earliest-offset tie-break，不包含更宽的 wrapper-family weighting、mixed-separator family 打分或 tokenizer 规则。
4. 复现当前基线建议命令：
   - `M8.5.44 focused RED→GREEN`：`.venv/bin/pytest tests/services/test_terminal_sessions.py -k "tab_prefixed_payload_offset_tie_break or prefers_later_tab_prefixed_payload" -q`
   - `M8.5.44 terminal focused baseline`：`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`
...
> 当前工作树已包含 `M8.5.44` 的 backend-only plain exact-tag tab-prefixed payload earliest-offset tie-break 优化并通过全量回归；下一自然入口是保持兼容边界不变的另一类窄范围 non-marker exact-tag payloadful separator 微调。
