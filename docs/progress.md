...
当前分支: `main`
最新 planning-only 提交: `f2bdf11` (`docs(plans): add M8.5.67 terminal archive filter reset UX plan`)
最新功能状态: 当前工作树已完成 `M8.5.67` terminal archive filter reset UX
...
- 最新可执行功能状态已推进到 `M8.5.67`：`/terminals` summary 区现在会明确提示 top-level archive export 当前是否处于过滤态，并新增 `Clear Archive Filters` 操作，一键将 archive-only 的 workspace/name/chat/status/owner/session/time 过滤状态重置回默认值；仍然只影响 `Export History Archive JSON`，不改变任何 backend contract。
- `M8.5.67` 继续保持 additive 边界：
...
- `docs/plans/2026-04-12-m8-5-67-terminal-archive-filter-reset-ux-design.md`
- `docs/plans/2026-04-12-m8-5-67-terminal-archive-filter-reset-ux.md`

当前 terminal 线额外注意点：

- `M8.5.67` 不新增任何顶层 archive 过滤参数，只补 archive-only UX 缺口
...
- `/terminals` summary 区的 archive-only 过滤输入现在包含 `Workspace Name Prefix`、`Workspace Prefix`、`Chat Access`、五态 `Status` 和既有 owner/session/time 输入，并额外提供过滤态提示与 `Clear Archive Filters` 操作；只有 `Export History Archive JSON` 使用它们，overview 和 latest-history 继续保持无过滤语义
...
2. 如继续 terminal 线，先区分两类工作：
   ...
   - operator surface：当前已经具备 overview 导出、latest-history bundle、带 `group_name_prefix`/`group_id_prefix`/`chat_accessible`/`status`/owner/session/time 过滤且支持一键 reset 的 cross-workspace history archive、detail 导出、timeline current-page bulk JSON 导出、search current-page JSON 导出、timeline all-pages archive JSON 导出、search all-pages archive JSON 导出
...
> `M8.5.67` 已在保持 grouped archive、RBAC、filename、backend/API contracts 和 workspace-scoped history contract 不变的前提下，为 `/terminals` 顶层 archive-only 过滤补齐 active-filter 提示和一键 reset UX；terminal relevance baseline 仍保持收敛暂停，除非出现新的真实证据。
