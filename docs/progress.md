...
当前分支: `feat/m8-5-15-terminal-preset-time-ranges-mainline`
最新功能提交: `b52697f` (`feat(web): add M8.5.15 terminal preset time ranges`)
...
- `M8.5.15` 已完成（feature branch：`feat/m8-5-15-terminal-preset-time-ranges-mainline`，`/terminals` 新增 `1h` / `6h` / `24h` / `7d` / `30d` preset 时间范围快捷筛选，前端复用既有 `snapshot_from` / `snapshot_to` 契约，无后端改动）。
- 当前起点：当前工作树代码基线已到 `M8.5.15`；下一步建议进入 post-`M8.5.15` 的搜索体验增强（例如可选结果排序策略），同时保持 `latest.json`、`/sessions/current/history` 与 RBAC 兼容边界不变。正式 `docs/TODO.md` 仍停在 `M6.5.3`。

---

## 2. 最近完成

- `M8.5.15` implementation（on `feat/m8-5-15-terminal-preset-time-ranges-mainline`）：新增 `docs/plans/2026-03-17-m8-5-15-terminal-preset-time-ranges-design.md` 与 `docs/plans/2026-03-17-m8-5-15-terminal-preset-time-ranges.md`，将范围固定为“frontend-only preset time-range shortcuts”，明确不新增 backend query 参数、不改变 `latest.json` / `/sessions/current/history` 契约。
- `M8.5.15` implementation（on `feat/m8-5-15-terminal-preset-time-ranges-mainline`）：扩展 `web/src/pages/Terminals.tsx`，新增 `1h` / `6h` / `24h` / `7d` / `30d` preset 按钮，将快捷范围写回既有 `snapshotFromLocal` / `snapshotToLocal`，timeline 与 search 继续共享同一时间过滤状态。
- `M8.5.15` implementation（on `feat/m8-5-15-terminal-preset-time-ranges-mainline`）：新增 `activePresetId` 前端状态；切换 preset 时复用既有 timeline/search/detail reset 逻辑，手动修改 `Snapshot From` / `Snapshot To` 时清空 preset 选中态，避免 UI 与请求状态分叉。
- `M8.5.15` implementation（on `feat/m8-5-15-terminal-preset-time-ranges-mainline`）：里程碑实现提交为 `b52697f`（`feat(web): add M8.5.15 terminal preset time ranges`）。
- `M8.5.15` implementation（on `feat/m8-5-15-terminal-preset-time-ranges-mainline`）：fresh 验证已通过 `cd web && npm run build`（frontend RED：preset helpers/type 未定义）-> `cd web && npm run build`（GREEN）、`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`、`.venv/bin/pytest -o addopts='' -q`（`611 passed`）、`.venv/bin/ruff check .`、`cd web && npm run lint`、`cd web && npm run build` 与 `git diff --check`。
...
- `M8.5.15` 当前位于 feature branch：terminal 搜索已新增 preset 时间范围快捷筛选，且仍复用既有 `snapshot_from` / `snapshot_to` 请求契约；如后续继续，应从排序/结果优先级体验入手，而不是重复扩张时间过滤协议。
...
2. 当前工作树代码基线已到 `M8.5.15`。terminal 搜索相关关键点在 `services/terminal_sessions.py`（时间范围 + metadata filters）、`app/routes/terminals.py`（timeline/search query 映射）、`web/src/api/client.ts` / `web/src/hooks/useApi.ts`（timeline/search filter request + cache key）、`web/src/pages/Terminals.tsx`（共享 filter state、`datetime-local` 输入、UTC 转换、preset 时间范围按钮与 `activePresetId` UI 状态）。
3. 如继续 post-`M8.5.15` terminal 开发，建议进入搜索体验增强的下一小步（例如可选结果排序策略），并保持 `latest.json` 与 `/sessions/current/history` 兼容、不扩展权限边界、不引入全文索引。
4. 复现当前基线建议命令：
   - `M8.5.15 focused`：`.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`
...
> 当前工作树已到 `M8.5.15`：terminal 搜索已具备分页、跨 session 导航、snippet 深链、metadata filters、时间范围过滤和 preset 时间范围快捷筛选，下一自然入口是结果排序/优先级体验增强。
