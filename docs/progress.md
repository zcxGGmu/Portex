...
- 当前起点：继续 parity backlog 时，从 `M7.5.3` 开始；正式 `docs/TODO.md` 仍停在 `M6.5.3`。
...
- `M7` 当前仍是 tasks/backlog 层路线，而不是 `docs/TODO.md` 的正式主计划；`M7.5.2` 已完成，下一步入口应前移到 `M7.5.3`，继续补更丰富的 room/workspace 切换 UX。
...
2. `M7.5.2` 已完成，关键实现点在 `web/src/components/chat/ChatPanel.tsx`、`web/src/api/client.ts`、`web/src/index.css`，以及规划文档 `docs/plans/2026-03-15-m7-5-2-chat-attachments-design.md` / `docs/plans/2026-03-15-m7-5-2-chat-attachments.md`。
3. 当前有效起点前移到 `M7.5.3`。继续 parity 时优先补更丰富的 room/workspace switching UX，不要提前吞掉 `M7.5.4` 的 IM binding UX。
4. 复现当前基线建议命令：
   - `M7.5.2 focused`：`cd web && npm run lint && npm run build`
...
> `M6`、`M7.1`、`M7.2`、`M7.3.1` ~ `M7.3.6`、`M7.4.1`、`M7.4.2`、`M7.4.3`、`M7.4.4`、`M7.4.5`、`M7.4.6`、`M7.4.7`、`M7.5.1`、`M7.5.2` 已完成；当前 parity backlog 的下一步入口是 `M7.5.3`。
