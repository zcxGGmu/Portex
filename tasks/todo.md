...
## Goal
- Continue post-`M8.5.65` terminal operator development by adding an optional `group_name_prefix` filter to the top-level cross-workspace history archive export on `/terminals`, with a matching archive-only control in the summary area, while keeping grouped archive items, snapshot-level history filters, and workspace-scoped history routes unchanged.
...
- [x] Validate the next filter choice: `backend` remains single-valued and `group_id_prefix` is already covered, so the next useful additive workspace boundary is human-facing `group_name_prefix`
...
- [x] Add this session checklist before implementation
- [x] Add failing route/OpenAPI coverage for top-level archive `group_id_prefix`
- [x] Implement filtered top-level archive route behavior for `group_id_prefix`
- [x] Add `/terminals` top-level archive `group_id_prefix` UI and client wiring
- [x] Run focused regression plus frontend lint/build and diff hygiene
- [x] Update `docs/progress.md`, `AGENTS.md` if needed, and complete this review section
- [x] Commit milestone changes with a detailed message

## Review
- Added RED coverage in `tests/app/routes/test_terminal_routes.py` and `tests/app/routes/test_api_routes.py` for grouped archive `group_id_prefix` filtering, route-level workspace selection, `filters.group_id_prefix` response metadata, and OpenAPI parameter exposure.
- Extended `app/routes/terminals.py` so `GET /terminals/history/archive` accepts optional `group_id_prefix`, trims and normalizes it, filters canonical web workspaces by `group_id.startswith(prefix)` before grouped archive loading, and echoes `filters.group_id_prefix` without changing grouped item shape, filename, RBAC, or `404` empty-result behavior.
- Extended `web/src/api/client.ts` and `web/src/pages/Terminals.tsx` so the archive-only filter state and top-level summary controls now include a `Workspace Prefix` input; only `Export History Archive JSON` uses it, while overview and latest-history exports remain unfiltered.
- Kept `services/terminal_sessions.py` unchanged so snapshot-derived filtering remains owned by the service and `group_id_prefix` stays a route-only workspace-selection boundary.
- Verification executed:
  - `.venv/bin/pytest tests/app/routes/test_terminal_routes.py tests/app/routes/test_api_routes.py -q`
  - `.venv/bin/pytest tests/services/test_terminal_sessions.py tests/app/routes/test_terminal_monitor_routes.py tests/app/routes/test_terminal_routes.py tests/app/routes/test_terminal_websocket_routes.py tests/app/routes/test_api_routes.py -q`
  - `.venv/bin/ruff check .`
  ...
  - `cd web && npm run build`
  - `git diff --check`
- Planning commit completed in this session: `ffc894f` (`docs(plans): add M8.5.65 terminal history archive group-prefix filter plan`).
